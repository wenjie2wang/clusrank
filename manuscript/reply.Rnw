...
Point-by-point responses are as follows, with quoted comments from the
reviewers in \emph{italic}.
...
Thank you for precise summary of our work and the remarks.
...
In \citet{wilcoxon1945individual}, the original rank sum test is to test that the two
distributions have the same means versus not. Later on the test was formulated
with null hypothesis that the two population have the same distributions
versus the alternatives that the two populations differ. For one sided tests,
the alternative hypothesis often states that one population is stochastically
larger than the other \citep{mann1947test}.
The test does not assume known distributions of these two populations and
therefore it was referred to as a nonparametric test \citep{fay2010wilcoxon}.
Note that, instead of a location shift in distributions, the Mann--Whitney form of
the Wilcoxon test on the basis of U-statistics is comparing the medians of the
two populations. The R function \texttt{wilcox.test} (for independent data)
formulates the alternative hypothesis as a location shift.
We have added a comment to the end of paragraph~1, Section~2.
...
The rationale of the test is based on checking whether it is equally likely
that a randomly 
...
@
The tests that we implemted in this package are extensions of the built-in R
function \texttt{wilcox.test} to handle clustered data.
...
give contradictory conclusions. The text has been rephrased.

...
Besides the asymptotic test for which the \texttt{clusrank} and
\texttt{ClusterRankTest} have the same implementation, \texttt{clusrank}
also provides a random permutation test for the small-sample cases.
A comment is added to the end of paragraph~5 in the Usage section.
...
clusWilcox.test(x ~ grp + cluster(cid), dat = dat.cl, method = "ds", exact = TRUE, B = 100)
...
clusWilcox.test(x ~ cluster(cid),  dat.sgn, paired = TRUE, method = "ds", exact = TRUE, B = 100)
@
...
one place approaches for nonparametric two sample comparison with clustered

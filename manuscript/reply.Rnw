...
location shift but may has no power in other situations such as a scale change.
For example, it has no power to distinguish $N(0, 1)$ and $N(0, 2)$ even with
...
Thxe data generating schemes were adapted from \citet{Datt:Satt:rank:2005},
which generate log-normal variables with a possible location shift.
...
The simulation study has been moved to the Appendix.

The current comparison is under skewed distribution.

All methods can deal with ties. 
\red{This sentence is unclear. Rephrase.}
Nevertheless, the data property that may
...
A comment has been added to paragraph ??? in the Discussion section.
...
They are two independent implementations of the same tests and should give the
same results when applied to the same data. Under the hood, however, our
implemenaton puts some computationally heavy steps into C++ code while their
code are purely R. Therefore, ours is faster:
<<ds-speed>>=
# do a speed comparison for a few sample sizes using the simulation
# setting in the paper using microbenchmark
# clusWilcox.test(...)
@ 
A comment has been added to the end of paragraph~1 in the Discussion section.
...
one place approches for nonparametric two sample comparison with clustered
...
This preceedent sentence has been rephrased for clarification.
...
\red{Too vague. Is there a difference between 10 and 11? does it depends on
  cluster size? Give some numbers like how long you need to wait.}
...
Thanks for this suggestion. \red{Can the RGL method be extended to handle
  this? }
We have added a warning to remind the user about
...
\red{What did you do to the text?}
...
\red{I don't see clusrank-package.Rd.}

...
two tests have been proposed by \citet{rosner2003incorporation}
and \citet{rosner2006wilcoxon}. 
...
Clustering often occurs in medical studies when the research 
objects are eyes, ears, teeth or in family studies. 
In these studies, an assumption usually being made is that
the outcome variables follow a normal or binomial distribution,
and tests based on this assumption have been studied intensively.
Nonetheless, under situations where these assumptions are
susceptible, blindly apply these test will lead to 
untrustworthy result. 


Usually, when the distribution underlying
the data is unknown or non-normal, one solution is to use
non-parametric test, e.g., the Wilcoxon rank sum test 
to compare the mean of two populations and the Wilcoxon 
signed rank test to test the shift of the population 
before and after a treatment. In the presence of 
clutering effect, the standard statistical procedures
will often lead to an underestimated p-value, because
the correlation within a cluster is usually positive. 


To adjust for clustering effect in non-parametric tests,
\citet{Rosn:GLyn:Lee:inco:2003} proposed a modified Wilcoxon
rank sum test 
and \citet{Roxn:Glyn:Lee:wilc:2006} poposed a modified Wilcoxon
signed rank test. The null distribution of both test statistics
are based on the asymptotic normality, therefore 
they are large sample tests. When the sample size is small,
permutation test could be applied. These two tests 
also allow unbalanced data, i.e., cluster sizes vary. 
Moreover, for Wilcoxon rank sum test, the input data can 
also be stratified, and stratified data is common in biomedical 
studies, e.g, studies carried out at different centers simultaneously
will generate data stratified by covariate center. 

...
Nonparametric tests are popular alternatives of
parametric tests when the underlying distribution of the
response variable is unknown or among the distributions
whose test procedures have not been derived specifically.
Standard non-parametric tests rely on the independence of
observations, however, often times the data come in
are clustered by nature or by the design of data collecting
scheme. Usually, but not always, the observations within
each cluster is positively correlated whereas different clusters
are independent. Therefore, the variance of the
test statistic of the standard non-parametric test
would be underestimated and thus
the power of the test would be lower. The following
test procedures adjust the variance used in the test
for the clustering effect and takes the effect of
different cluster sizes as well.
...
\subsection{Balanced designs}\label{ranksumbal}
...
Data \text{sedlab} is provided by Lab Seddon Lab
\citep{ferrara2015pheno}.%% Note: If there is markup in \(sub)section, then it has to be escape as above.

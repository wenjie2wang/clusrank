...
Implementations of Wilcoxon rank-sum and signed rank tests have been
available in standard software packages, such as \proglang{R} package
\pkg{stats}, and \proglang{SAS PROC RANK}.
Nonetheless, the existing implementations assume independent data,
which cannot be applied to clustered data arising in many fields. 
Clustered data consists of data from clusters, where clusters are
mutually independent but there is dependence within each cluster.
...
As a consequence, the resulting p-values would be misleading.


Several recent developments have extended the Wilcoxon rank-sum test
...
A large sample test statistic for clustered data by conditioning on the cluster size was introduced in
...
\citet{Rosn:Glyn:Lee:exte:2006} extended their approach to accommodate the

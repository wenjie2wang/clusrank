...
\citet{Datt:Satt:rank:2005} to generate sample for clustered
...
being added to the fixed effect.
...
As an illustration, a balanced data set contains $10$ cluster is
generated with treatment assigned at cluster level.
<<dat-clus>>=
set.seed(616)
dat.cls <- datgen.rs(10, 5, rho=c(.9, .9), grplevel="cluster")
head(dat.cls)
@ 
To apply RGL method on this dataset with asymptotic test 
and exact test:
<<clus-rs-rgl>>=
cluswilcox.test(score ~ grp + cluster(id), data = dat.cls, method = "rgl")
cluswilcox.test(score ~ grp + cluster(id), data = dat.cls, method = "rgl", 
                exact = TRUE)
@ 
...
<<clus-rs-indi>>=
cluswilcox.test(score ~ grp + cluster(id), data = dat.ind, method = "rgl")
cluswilcox.test(score ~ grp + cluster(id), data = dat.ind, method = "ds")
@ 
...
<<sr>>=
cluswilcox.test(score ~ cluster(id), dat.sr, paired=TRUE, method="rgl")
cluswilcox.test(score ~ cluster(id), dat.sr, paired = TRUE, method = "ds")
@ 
...
The table \ref{tab:rs} includes results from $10$ scenarios to compare
the performance of the two methods of clustered rank-sum test. The
...
\begin{table}[ht]
\centering
\caption{Simulation for clustered rank sum test, comparsion between RGL and DS} 
\begin{tabular}{rrrrr}
  \toprule
   & \multicolumn{2}{c}{size} & \multicolumn{2}{c}{power} \\
 & RGL & DS & RGL & DS \\
 \midrule
clus, bal, $\rho = 0.1$ & 0.04 & 0.05 & 0.62 & 0.65 \\ 
  clus, bal, $\rho = 0.9$ & 0.06 & 0.07 & 0.26 & 0.28 \\ 
  clus, unbal, $\rho = 0.1$ & 0.04 & 0.04 & 0.32 & 0.39 \\ 
  clus, unbal, $\rho = 0.9$ & 0.05 & 0.06 & 0.20 & 0.25 \\ 

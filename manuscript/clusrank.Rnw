...
\Abstract{
  Nonparametric tests based on rank are popular alternatives to
  two-sample and paired $t$-tests.
For independent data, they are available in
the built-in \pkg{stats} package of \proglang{R}.
For clustered data, several rank-based tests that consider
...
Permutation tests for small sample are provided for some methods.
Details of the major methods are briefly reviewed.
Usages of the main function are illustrated with simulated data.
The package also enables convenient comparison between selected methods
under settings that have not been studied before and the results are discussed.
}
...
Wilcoxon rank-sum and signed-rank tests are important tools
to detect the difference in locations of two independent or dependent
groups, respectively.
...
Clustered data consists of independent clusters of dependent
measurements.
...
therefore the variance of the test statistics derived under the
independence assumption are invalid.
...
test to allow two sample comparison for clustered data.
...
\citet{Rosn:Glyn:Lee:exte:2006} extended their previous approach to
accommodate the situation where members of a single cluster may
subject to different treatments, but the new test still assumes
exchangeability among
cluster members and identical intracluster dependence across groups.
...
For the one-sample or paired comparison problem,
...
applications such as biomedicine and social science,
...
Modeled after the familiar function \code{wilcox.test}, package \pkg{clusrank}
unifies both RGL and DS methods under a user-friendly
interface that accommodates the specifications from these methods.
Our package makes it very easy to compare the performances of
...
Like all permutation methods, the RGL method assumes that the
...
Since the RGL method compares groups at each cluster size, the
...
If only one group shows up at a cluster size, this part of the data
will be ignored.
...
dependence is weak; otherwise, since the effective number of
...
Let $X_i^*$ be a random pick from the $i$th cluster in a
...
The construction of the test statistic allows no room for
...
usually an traditional assumption made by most signed-rank tests for
clustered data in literature.
...
certain scenarios have not been studied in the recent literature.
...
\begin{table}[tbp]
  \centering
   \caption{
     Empirical rejection percentage of the RGL method and the DS
     method for signed-rank tests at nominal significance level 0.05.
...
The empirical power of both tests increases as cluster size increase,
...
individual in each group within a cluster \citep{Rosn:Glyn:Lee:exte:2006};
...
For rank-sum test, the case of informative group size within a cluster
...
Sign test and signed-rank tests for multivariate clustered data

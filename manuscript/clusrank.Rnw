...
offering great flexibility in handling various situations such as stratified data,  
...
with illustrations via simulated data.
...
number of subjects under treatment is associated with the cluster
size. 
...
\proglang{R} package \pkg{ClusterRankTest} by the authors
...
the different approaches under various settings. We also acknowledge
the R code and SAS code provided by the authors on their website \citep{SAS:inco:2016, SAS:sub:2016, SAS:sign:2016, R:clus:2016}.

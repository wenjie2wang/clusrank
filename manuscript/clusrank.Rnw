...
@
Depending on whether \code{paired} is set to be \code{TRUE} or
\code{FALSE}, clustered rank-sum test or the clustered signed-rank
test is performed, respectively.

{\bf describe the arguments that are inherited from \code{wilcox.test}
  first and then describe those extra that are specific for clustered version}

For clustered rank-sum test, the data frame \code{data} should
at least include $3$ variables: the observed score, \code{x},
the cluster id, \code{cluster} and the treatment group, \code{group}.
The argument \code{y} is reserved for clustered signed-rank test. 
{\bf need to match RGL with rgl and DS with ds}
Both RGL and DS methods are available using asymptotic statistic. 
Note that for RGL method, an extra varaible \code{stratum} can be
supplied to account for the effect of stratification in the data.
In addition, for RGL method, when the treatment group is assigned at cluster
level, an exact test is available by setting \code{exact ==TRUE}.
Nevertheless, exact test is not recommended for data with number of
clusters is greater than $50$ due to computational issue. 
The other arguments, \code{alternative} and \code{mu}, set the shape of the
alternative hypothesis and the null value respectively. 

Clustered signed-rank test is run if \code{paired} is set as
\code{TRUE}. There are two ways to supply the response variable:
supply the observed difference as \code{x}, or supply the pair of
observed scores as \code{x} and \code{y} respectively.  
Same as
clustered rank-sum test, exact test is only available for RGL method.
The other arguments are set in the same way as for the clustered
rank-sum test.  Note that there is no available method that can
control the effect of stratification.
 

The arguments of the formula interface is as the following:
<<formula, echo=FALSE>>=
...
For rank-sum test, the left hand side of the formula is the observed
score and the right hand side comprises the required covariates
including \code{cluster} id and \code{group} id (see section~\ref{s:illus} for detail). 
Additional covariate for specific method can also be added, e.g., \code{stratum} for
RGL method.


{\bf watch line width}
For signed-rank test, the left hand site of the formula is the 
observed paired difference and the right hand side comprises the cluster id, note that no group id is needed for signed-rank test.
The \code{subset} and \code{na.action} have the same effect as they 
have in the \code{model.frame} function. Other arguments are the
same as those in the default interface.
...
{\bf what is the reference of this simulation scheme?}

{\bf watch line width? what editor are you using?}

The following function generates clustered data (balanced or unbalanced) from two treatment groups.
<<datgen, echo=TRUE>>= 
library(mvtnorm)
...
When \code{grplevel} is set as \code{"cluster"}, the generated data frame contains
\code{nclus} clusters under each treatment group with corresponding
intra-cluster correlation; whereas if \code{grplevel} set as
\code{"individual"}, a data set with individual level grouping is
obtained by randomly re-assigning the treatment id at indivudual level. The maximum cluster size is set as
\code{maxclsize}. 

{\bf no need for another paragraph. Watch line widht}

Argument \code{rate} is used to control the unbalancedness of the data by randomly
keeping a proportion of \code{rate} of the orignal balanced data where each 
cluster has the size \code{maxclsize}.
Therefore, to genearte balanced data
the \code{rate} can simply be set to $1$.
The intra-cluster correlation of each treatment group is set through
\code{rho} which also
enables a one-to-one correspondence between the treatment and the 
magnitude of intra-cluster correlation.
The shift in the location of the two treatment groups 
is set by \code{delta}. 
The last argument, \code{grplevel}, is a switch of the grouping level:
if \code{grplevel = TRUE}, the grouping is assigned at cluster level, 
otherwise, it is assigned at individual level. The generated data set contains three columns: \code{score}, \code{grp} and \code{id} which correspond to the observation, treatment group id and cluster id.


{\bf please use two blank lines to separate paragraphs}
The first dataset has 10 clusters in each of the two groups, 
and the grouping is at the cluster level.
<<dat-clus>>=
set.seed(616)
dat0.cls <- datgen.rs(10, 5, rho=c(.9, .9), grplevel="cluster")
head(dat0.cls)
@ 
To apply RGL method on this dataset with asymptotic test 

{\bf the group in group(grp) is not needed. You always have a right
  hand side variable, which is your group. Cluster and strata can be
  specified using cluster and strata, but no need for group.}
and exact test:
<<clus-rs-rgl>>=
cluswilcox.test(score ~ group(grp) + cluster(id), data = dat0.cls, method = "rgl")
cluswilcox.test(score ~ group(grp) + cluster(id), data = dat0.cls, method = "rgl", 
                exact = TRUE)
@ 
To apply DS method on this dataset:
<<clus-rs-ds>>=
cluswilcox.test(score ~ group(grp) + cluster(id), data = dat0.cls, method = "ds")
@ 
...
<<dat-rs-rgl-indi>>=
dat1.cls <- datgen.rs(10, 5, rho=c(.9, .9), delta = 1, grplevel="individual")
head(dat1.cls)
@ 
Apply the clustered signed-rank test on the dataset using the two available methods:
<<clus-rs-indi>>=
cluswilcox.test(score ~ group(grp) + cluster(id), data = dat1.cls, method = "rgl")
cluswilcox.test(score ~ group(grp) + cluster(id), data = dat1.cls, method = "ds")
@ 



A simulation to compare the two methods can be easily done:
{\bf this function looks clumsy. Might be better to separate it into two for readability}
<<simpower>>=
simpower <- function(nrep, level, paired, delta, rho, rate, nclus, maxclsize, ...) {
    do1rep <- function() {
        if(paired) datgen <- datgen.sr
        else datgen <- datgen.rs
        dat <- datgen(nclus, maxclsize, delta, rho, rate, ...);
        if( !("grp" %in% colnames(dat))) { # Signed-rank test
            p.rgl <- cluswilcox.test(score, cluster = id, paired = TRUE, 
                                     data = dat, method = "rgl")$p.value
            p.ds  <- cluswilcox.test(score, cluster = id, paired = TRUE, 
...
Following code compares the RGL and DS methods of the clustered rank-sum test
using data where grouping is assigned at cluster level. The simulated
balanced data sets contains $10$ clusters with maximum cluster size
$5$. Since the unbalanced data sets are obtained by introducing random
missingness in the balanced data sets, the number of clusters is no
more than $10$ and each cluster size is no more than $5$. In addition,
the intra-cluster correlation is identitical for all clusters. 
<<size-clus-bal>>=
simpower(nrep = 1000, level = 0.05, paired = FALSE, delta = 0, 
         rho = c(0.1, 0.1), rate = 1, nclus = 10, maxclsize = 5, "cluster")
simpower(1000, 0.05, FALSE, 0, c(0.9, 0.9), 1, 10, 5, "cluster")
@
When the data set is balanced (\code{rate} = 1), the empirical sizes of
the two methods are close to the nominal size $0.05$ despite of the
magnitude of the intra-cluster correlation.

<<power-clus-bal>>=
simpower(1000, 0.05, FALSE, 0.5, c(0.1, 0.1), 1, 10, 5, "cluster")
simpower(1000, 0.05, FALSE, 0.5, c(0.9, 0.9), 1, 10, 5, "cluster")
@
When the true difference in the location is $0.5$, the power of both
methods is comparable, and decreases as the intra-cluster correlation
increases. 

{\bf These experiments should be done outside of the manuscript.
  Put them in a r markdown file; study them and make your selection.
  In the manuscript you present only those you selected to present.}

<<size-clus-bal-diff>>=
simpower(1000, 0.05, FALSE, 0, c(0.1, 0.9), 1, 10, 5, "cluster")
simpower(1000, 0.05, FALSE, 0, c(0.5, 0.9), 1, 10, 5, "cluster")
simpower(1000, 0.05, FALSE, 0, c(0.5, 0.1), 1, 10, 5, "cluster")
@


<<power-clus-bal-diff>>=
simpower(1000, 0.05, FALSE, 0.5, c(0.1, 0.9), 1, 10, 5, "cluster")
simpower(1000, 0.05, FALSE, 0.5, c(0.5, 0.9), 1, 10, 5, "cluster")
simpower(1000, 0.05, FALSE, 0.5, c(0.5, 0.1), 1, 10, 5, "cluster")
@

<<size-clus-bal-diff-10>>=
simpower(1000, 0.05, FALSE, 0, c(0.1, 0.9), 1, 10, 10, "cluster")
simpower(1000, 0.05, FALSE, 0, c(0.5, 0.9), 1, 10, 10, "cluster")
simpower(1000, 0.05, FALSE, 0, c(0.5, 0.1), 1, 10, 10, "cluster")
@


<<power-clus-bal-diff-10>>=
simpower(1000, 0.05, FALSE, 0.5, c(0.1, 0.9), 1, 10, 10, "cluster")
simpower(1000, 0.05, FALSE, 0.5, c(0.5, 0.9), 1, 10, 10, "cluster")
simpower(1000, 0.05, FALSE, 0.5, c(0.5, 0.1), 1, 10, 10, "cluster")
@


<<size-ind-bal-diff>>=
simpower(1000, 0.05, FALSE, 0, c(0.1, 0.9), 1, 10, 5, "individual")
simpower(1000, 0.05, FALSE, 0, c(0.5, 0.9), 1, 10, 5, "individual")
simpower(1000, 0.05, FALSE, 0, c(0.5, 0.1), 1, 10, 5, "individual")
@


<<power-ind-bal-diff>>=
simpower(1000, 0.05, FALSE, 0.5, c(0.1, 0.9), 1, 10, 5, "individual")
simpower(1000, 0.05, FALSE, 0.5, c(0.5, 0.9), 1, 10, 5, "individual")
simpower(1000, 0.05, FALSE, 0.5, c(0.5, 0.1), 1, 10, 5, "individual")
@



<<size-ind-unbal-diff>>=
simpower(1000, 0.05, FALSE, 0, c(0.1, 0.9), 0.5, 10, 5, "individual")
simpower(1000, 0.05, FALSE, 0, c(0.5, 0.9), 0.5, 10, 5, "individual")
simpower(1000, 0.05, FALSE, 0, c(0.5, 0.1), 0.5, 10, 5, "individual")
@


<<power-ind-unbal-diff>>=
simpower(1000, 0.05, FALSE, 0.5, c(0.1, 0.9), 0.5, 10, 5, "individual")
simpower(1000, 0.05, FALSE, 0.5, c(0.5, 0.9), 0.5, 10, 5, "individual")
simpower(1000, 0.05, FALSE, 0.5, c(0.5, 0.1), 0.5, 10, 5, "individual")
@


<<size-ind-bal-diff-10>>=
simpower(1000, 0.05, FALSE, 0, c(0.1, 0.9), 1, 10,10, "individual")
simpower(1000, 0.05, FALSE, 0, c(0.5, 0.9), 1, 10,10, "individual")
simpower(1000, 0.05, FALSE, 0, c(0.5, 0.1), 1, 10,10, "individual")
@


<<power-ind-bal-diff-10>>=
simpower(1000, 0.05, FALSE, 0.5, c(0.1, 0.9), 1, 10,10, "individual")
simpower(1000, 0.05, FALSE, 0.5, c(0.5, 0.9), 1, 10,10, "individual")
simpower(1000, 0.05, FALSE, 0.5, c(0.5, 0.1), 1, 10,10, "individual")
@



<<size-ind-unbal-diff-10>>=
simpower(1000, 0.05, FALSE, 0, c(0.1, 0.9), 0.5, 10,10, "individual")
simpower(1000, 0.05, FALSE, 0, c(0.5, 0.9), 0.5, 10,10, "individual")
simpower(1000, 0.05, FALSE, 0, c(0.5, 0.1), 0.5, 10,10, "individual")
@


<<power-ind-unbal-diff-10>>=
simpower(1000, 0.05, FALSE, 0.5, c(0.1, 0.9), 0.5, 10,10, "individual")
simpower(1000, 0.05, FALSE, 0.5, c(0.5, 0.9), 0.5, 10,10, "individual")
simpower(1000, 0.05, FALSE, 0.5, c(0.5, 0.1), 0.5, 10,10, "individual")
@


<<size-clus-unbal>>=
simpower(1000, 0.05, FALSE, 0, c(0.1, 0.1), 0.5, 10, 5, "cluster")
simpower(1000, 0.05, FALSE, 0, c(0.9, 0.9), 0.5, 10, 5, "cluster")
@
For unbalanced data which are generated by randomly deleting
half of a balanced data set, 
the RGL method is still able to be close to the nominal size, 
whereas the empirical size of DS
{\bf is this setting what DS is designed for?}
method is more than two folds of the nominal size and deviates more
from the nominal size when the intra-cluster correlation is low.

<<power-clus-unbal>>=
simpower(1000, 0.05, FALSE, 0.5, c(0.1, 0.1), 0.5, 10, 5, "cluster")
simpower(1000, 0.05, FALSE, 0.5, c(0.9, 0.9), 0.5, 10, 5, "cluster")
@

The empirical power of RGL method is less than that of the DS method,
however, the DS method does not have the correct size in this case.

<<size-clus-unbal-10>>=
simpower(1000, 0.05, FALSE, 0, c(0.1, 0.1), 0.5, 10, 10, "cluster")
simpower(1000, 0.05, FALSE, 0, c(0.9, 0.9), 0.5, 10, 10, "cluster")
@

When the maximum cluster size is increased from $5$ to $10$, where
more variability in the cluster size is introduced,  the empirical
size of both methods is close to the nominal level for the unbalanced data.

<<power-clus-unbal-10>>=
simpower(1000, 0.05, FALSE, 0.5, c(0.1, 0.1), 0.5, 10, 10, "cluster")
simpower(1000, 0.05, FALSE, 0.5, c(0.9, 0.9), 0.5, 10, 10, "cluster")
@
For the same setting, the empirical power of RGL method is less than
that of  DS method despite of the magnitude of the intra-cluster
correlation. 
<<size-ind>>=
simpower(1000, 0.05, FALSE, 0, c(0.1, 0.1), 1, 10, 5, "individual")
simpower(1000, 0.05, FALSE, 0, c(0.9, 0.9), 1, 10, 5, "individual")
simpower(1000, 0.05, FALSE, 0, c(0.1, 0.1), 0.5, 10, 5, "individual")
simpower(1000, 0.05, FALSE, 0, c(0.9, 0.9), 0.5, 10, 5, "individual")
@
Under a similar setting with treatment assigned at individual level, 
it is also observed that with balanced data, both methods can attain
the norminal size, whereas when the data is unbalanced by randomly
deleting half of the observations, the DS method cannot attain the
nominal size. {\bf Make sure what DS method is designed for.}

{\bf watch line width}


<<power-ind-bal>>=
simpower(1000, 0.05, FALSE, 0.5, c(0.1, 0.1), 1, 10, 5, "individual")
simpower(1000, 0.05, FALSE, 0.5, c(0.9, 0.9), 1, 10, 5, "individual")
@

For balanced data, the empirical power of RGL method is higher
than that of DS method regardless of the strength of the intra-cluster correlation.

<<power-ind-unbal>>=
simpower(1000, 0.05, FALSE, 0.5, c(0.1, 0.1), 0.5, 10, 5, "individual")
simpower(1000, 0.05, FALSE, 0.5, c(0.9, 0.9), 0.5, 10, 5, "individual")
@

For unbalanced data, the empirical power of RGL method is close to
that of the DS method, however, as the intra-cluster correlation
increases, RGL method is increasingly more powerful than DS method.

<<size-ind-unbal-10>>=
simpower(1000, 0.05, FALSE, 0, c(0.1, 0.1), 0.5, 10, 10, "individual")
simpower(1000, 0.05, FALSE, 0, c(0.9, 0.9), 0.5, 10, 10, "individual")
@

If maximum cluster size is increased from $5$ to $10$, similar to the
simulation result when treatment group is assigned at cluster level,
both methods retain size close to the nominal level.

<<power-ind-unbal-10>>=
simpower(1000, 0.05, FALSE, 0.5, c(0.1, 0.1), 0.5, 10, 10, "individual")
simpower(1000, 0.05, FALSE, 0.5, c(0.9, 0.9), 0.5, 10, 10, "individual")
@


<<size-ind-unbal-5-20>>=
simpower(1000, 0.05, FALSE, 0, c(0.1, 0.1), 0.5, 20, 5, "individual")
simpower(1000, 0.05, FALSE, 0, c(0.9, 0.9), 0.5, 20, 5, "individual")
@
<<power-ind-unbal-5-20>>=
simpower(1000, 0.05, FALSE, 0.5, c(0.1, 0.1), 0.5, 20, 5, "individual")
simpower(1000, 0.05, FALSE, 0.5, c(0.9, 0.9), 0.5, 20, 5, "individual")
@


<<size-ind-unbal-5-20-8>>=
simpower(1000, 0.05, FALSE, 0, c(0.1, 0.1), 0.8, 20, 5, "individual")
simpower(1000, 0.05, FALSE, 0, c(0.9, 0.9), 0.8, 20, 5, "individual")
@
<<power-ind-unbal-5-20-8>>=
simpower(1000, 0.05, FALSE, 0.5, c(0.1, 0.1), 0.8, 20, 5, "individual")
simpower(1000, 0.05, FALSE, 0.5, c(0.9, 0.9), 0.8, 20, 5, "individual")
@

{\bf Please make an effor to select and present. The discussions from
  the DS and RGL methods should give us a hint on which method is
  better in what situation.}


With a low intra-cluster correlation, the empirical of both methods
are close, as the correlation increases, the RGL method becomes
increasingly more powerful than the DS method.

For unbalanced data sets which are obtained by randomly deleting half of
the observations from the balanced data, the empirical power of RGL
method is still higher than that of the DS method.  
Note that the size of the DS method is biased in this case. 
...
A size study with strong and weak intra-cluster correlation is as following:
<<size_sr>>=
simpower(1000, 0.05, TRUE, 0, 0.9, 1, 10, 5)
simpower(1000, 0.05, TRUE, 0, -0.1, 1, 10, 5)
@ 
The ``rgl'' method has an empirical size closer to the true size $0.05$ when the 
intra-cluster correlation is large. Both methods have a smaller empirical size
when intra-cluster correlation is slightly negative.

A power size under same setting with a shift equals $1$ is as the following:
<<power_sr>>=
simpower(1000, 0.05, TRUE, 1, 0.9, 1, 10, 5)
simpower(1000, 0.05, TRUE, 1, -0.1, 1, 10, 5)
@ 
In both cases, ``ds'' method has a higher power.  

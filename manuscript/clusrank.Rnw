...
requested by setting \code{exact = TRUE} and \code{B = 0}. However, the
exact tests can be very computing intensive and it is recommended to be used
only with small samples (will test the size of sample). The package also
...
Our implementation of the DS methods gives the same results as those in the
\pkg{ClusterRankTest} package. We put them together in order for the users
to have a more convenient access to different methods for the same test.

...
\code{rho} as follows, \code{ex} for exchangeable and \code{ar1} for AR1.
<<corstr>>=
ex  <- function(dim, rho) {
...
Here is a simple implementation that allows different levels of grouping 
(cluster-level and individual-level) and unequal cluster sizes. 
...
@ 
There are two required inputs: \code{nclus} for number of clusters 
...
@ 
Setting \code{method = "ds''} would preform the DS test.
...
To get the matching results, one just needs to switch the group numbers. 
...
We illustrate this by changing the \code{grp} in the dataset:
<<clus-rs-ds-mgrp>>=
dat.cl$grp <- rep(1:4, each = 15)
clusWilcox.test(x ~ grp + cluster(cid), dat = dat.cl, method = "ds")
@ 

\red{Why is this test so significant?}
...
We generate the data to illustrate clustered signed-rank tests using a
scheme slightly modified from simulation scenario~1 in 
\citet{Datt:Satt:sign:2008},
both balanced
...
datgen.sr <- function(nclus, maxclsize, delta = 0.,
    ...
    z <- c(t(mvnorm(nclus, sigma = Sigma)))
    x <- sign(z) * exp(abs(z))

...
Non-normal clustered data often arise in many fields, where the
clusters are independent but individuals within each cluster are not. 
For example, in ophthalmologic clinical trials, the outcomes of
interest are often ordinal such as the Early Treatment for Diabetic
Retinopathy Scale (ETDRS), and the individual measurements from
both eyes from the same subject (cluster) are not independent.
The goal of the studies are often to test whether two samples are from
the same population against certain alternative (e.g., one population
...
\textbf{What is this reference? why citing it?}
\citep{Donn:Bant:anal:1988}.
Standard nonparametric tests, such as Wilconxon rank-sum test
or signed rank test, are important tools for non-normal,
independent data, but they are not applicable to clustered data. 
Often, with positive within-cluster association, the effective
sample size is less than the number of individuals.
Consequently, the p-values from naive application of these tests
are underestimated and misleading conclusions could be reached.

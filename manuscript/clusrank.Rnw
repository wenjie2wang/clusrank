...
the two methods are close to the nominal size $0.05$ both with a high
intra-cluster correlation and with a low intra-cluster correlation.
...
methods are comparable, and decreases as the intra-cluster correlation
increases. 
<<size-clus-unbal>>=
simpower(1000, 0.05, FALSE, 0, c(0.1, 0.1), 0.5, 10, 5, "cluster")
simpower(1000, 0.05, FALSE, 0, c(0.9, 0.9), 0.5, 10, 5, "cluster")
@
For unbalanced data sets which are generated by randomly deleting
half of the balanced data as in the previous simulation, 
...
when the intra-cluster correlation is low as $0.1$ and high as
$0.9$. Note, however, the empirical size of DS method is biased in
this situation.
<<size-ind>>=
...
For unbalanced data, the empirical 
...
the observations from the balanced data, the empirical power of RGL method is still higher than that of the DS method. Note that the size of the DS method is biased in this case. 

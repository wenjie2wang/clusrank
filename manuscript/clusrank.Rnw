...
cluster & ex  & 0 & 2 & 0.1, 0.1 & 4.3 & 4.7 & 17.5 & 17.9 & 64.3 & 65.3 \\
 ...
 & ar1 & 0 & 2 & 0.1, 0.1 & 4.0 & 4.2 & 18.4 & 19.0 & 65.4 & 66.4 \\
  ...
  observation member  & ex  & 0 & 2 & 0.1, 0.1 & 4.0 & 4.3 & 17.5 & 16.7 & 69.2 & 67.8 \\
...
\citep{Ferr:Sedd:phen:2015}. The aim of the study was to characterize
the phenotype associated with a rare variant in the complement factor
H R1210C, a large protein. The measurements are taken on both eyes of
each subject, therefore the measurements are clustered.
The pheonotypic features under study are characterized by the
...
Additional information such as age and sex are also provided.
...
\code{Age_group}, and \code{Agesex (a composite score constructed from
  Sex and Age_group).

<<eye1234>>=
data(eye)
clusWilcox.test(CARMS ~ Variant + cluster(ID) + stratum(Agesex), data = eye, subset = CARMS %in% c(1, 2, 3, 4))
@ 


<<eye1235>>=
clusWilcox.test(CARMS ~ Variant + cluster(ID) + stratum(Agesex), data = eye, subset = CARMS %in% c(1, 2, 3, 5))
@ 

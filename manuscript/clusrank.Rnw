...
The formula interface works for both rank sum test and signed rank
test.  For rank sum test, the left hand side of the formula is the
observed score and the right hand side comprises the required
covariates including \code{cluster} id and \code{group} id.
Additional covariate for specific method can also be added, e.g.,
\code{stratum} for \code{rgl} method.  
The \code{data} argument needs the name of a data frame which should
consist of the observed score and required covaraites.  
...
\code{mu} and \code{exact}.
Note that the exact permutation tests are only available for 
rank sum test using \code{rgl} method when the treatment is assigned 
at cluster level and should be applied to sample with number of 
clusters less than 50.


Clustered signed rank test is performed when \code{paired} is set to
be \code{TRUE}. 
For clustered signed rank test, the variable on the left hand side 
of the formula should be the observed difference in
...
score after treatment is required as \code{y}.  All other arguments
can be set exactly the same as using the formula interface.
\red{FIXME: explain all the arguments; separate ranksum and signed
  rank.}


\red{FIXME: at the very end, give the default interface similar to
wilcox.test}
...
\code{nclus} determines the number of clusters to be generated,
\code{maxclsize} sets the maximum cluster size, \code{delta} sets the
difference in the location of the two treatment groups, \code{rho}
sets the intra-cluster correlaiton of each treatment group which
enables the one-to-one correspondence between treatment and
intra-cluster correlation structure. The function will first generate
a balanced data set and each cluster has size \code{maxclsize},
unbalancedness can be introduced by keeping \code{rate} of the whole
data randomly. Finally the \code{grplevel} is a switch of the level on
which the treatment is assigned.  The first datset has 10 clusters in
each of the two groups, and the grouping is at the cluster level.
<<dat-clus>>=
set.seed(616)
...
\red{FIXME: can a user specify \code{method = c('rgl', 'ds')} to
  request both tests in one call? No. Otherwise the default method is
  both instead of ``ds''}
...
\red{FIXME: can you do something similar? Use some simple function to
  generate data, illustrate usage of each function, and do some
  simulation that has not been done before if possible.}

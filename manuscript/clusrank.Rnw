...
observed measurement \code{score}, grouping id \code{grp}, and cluster
id \code{cid}.
...
<<clus-rs-ds>>=
clusWilcox.test(score, group =  grp, cluster = cid, data = dat.cls, method = "ds")
@ 
...
head(dat.str)
clusWilcox.test(score ~ grp + cluster(cid) + stratum(strat), dat = dat.str,  method = "rgl")
@ 
...
clusWilcox.test(score ~ grp + cluster(cid), dat = dat.m, method = "ds")
@ 
...
Using the two data generation functions, we provide a convenient
function, \code{simpower}, to facilitate the comparison of empirical
size and power of RGL and DS method for each test under a range of
scenarios.
...
<<sim-clus-rs>>=
simpower(1000, 0.05, FALSE, 10, 5, 0, c(0.1, 0.1), xch, 1, "cluster")
...
is assigned at individual level yet. 
...
&&&&& \multicolumn{2}{c}{$\delta = 0$} & \multicolumn{2}{c}{$\delta = 0.3$} & \multicolumn{2}{c}{$\delta = 0.6$} \\
  ...
  Design & Correlation structure & $\rho$ & M & rate & RGL & DS & RGL & DS & RGL & DS \\

  \midrule
1 & Exchangeable & 0.1 & 5 & 1 &  0.05 & 0.05 & 0.43 & 0.38 & 0.89 & 0.86 \\ 
...
The upper bound standard error of the empirical power can be estimated as $0.5 /

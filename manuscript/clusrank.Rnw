...
\code{wilcox.test}. The random permutation test (\code{perm = TRUE},
and \code{B} determines the size of the permutation sample) is
available for all tests, though exact permutation test is only
available for the RGL clustered rank-sum test with cluster-level
grouping and the RGL clustered signed-rank test (\code{perm = TRUE},
and \code{B = 1}); otherwise \code{perm} should be set as \code{FALSE}.
Since the permutation test (especially the exact test) is computationally very
...
The AR1 correlation structure within clusters is common in
longitudinal studies, and we choose it to demonstrate the performance
of the clustered tests when the intra-cluster correlation is not
exchangable. Correlation matrix of dimension \code{dim} with
correlation parameter \code{rho} can be generated as follows, with
\code{ex} for exchangeable and \code{ar1} for AR1.
<<corstr>>=
ex  <- function(dim, rho) {
...
\caption{Empirical rejection percentage of the RGL and the DS methods
  ...
  intracluster correlation is exchangable. Grouping is at cluster
  level. The results are based on 4000 datasets. Each group contains
...
\caption{Empirical rejection percentage of the RGL and the DS methods
  for rank-sum tests at nominal significance level 0.05 when
  intracluster correlation is exchangable. Grouping is at subunit
  level. The results are based on 4000 datasets. Each group contains
  same number of clusters.}
\centering
\begin{tabular}{ccccc rr rr rr}
...
two groups, each with 20, 50, or 100~clusters, and with exchangeable
intracluster correlation structure as specified in \code{datgen.sum}
(results for AR1 intracluster correlation is shown in the Appendix).
...
Selected results are summarized in Table~\ref{tab:rsex}.
...
The differences to be tested were generated for 20, 50, and
100~clusters, respectively.
...
(see Table~\ref{tab:srar} in the Appendix).

...
the arguments \code{nclus}, \code{maxclsize}, \code{delta}
and \code{rate} have the same usage as they have in \code{datgen}
for the rank sum test. Since unlike rank sum test which invovles two 
groups, there is no grouping for signed rank test, therefore
\code{rho} is a numerical which determines the magnitude of

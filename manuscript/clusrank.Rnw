...
{\bf this sentence is too long and confusing. need to break it into
  smaller ones and rephrase. You can say that y becomes optional
  because you have a group variable which allows more than 2 groups.}
...
The exact test (\code{exact = TRUE}) is only available for the RGL
clustered rank-sum test with cluster level grouping; otherwise
\code{exact} should be set as \code{FALSE}.  
{\bf DS method cannot have exact? or too hard to implement?}
Note that an exact test is very computing intensive, so it is
recommended to be used only with small sample.
{\bf do we decline or  issue a warning if number of cluster is greater 40?}
{\bf try to start a new sentence at a new line}


The function specifies the cluster by a \code{cluster} variable
indicating which cluster each observations belongs to.
Additionally, stratified data is allowed by a code{strata} variable
indicating which strata each observation belongs to.
{\bf rephrase...}
...
{\bf we designed the formula interface to wilcox.test. so
  present the same aspects first and then describe the additions.}
...
\code{wilcox.test}. 
Variables indicating clusters and strata are identified in the formula
by special terms \code{cluster} and \code{stratum}, respectively.
For example, \code{z ~ group + cluster(cid) + stratum(sid)}, 
where \code{group} identifies the grouping variable, \code{cid}
identifies the clusters, and \code{sid} identifies the strata.
...
cluster id. No group id is needed for signed-rank test. 
{\bf need to say that cluster and strata are not applicable}

{\bf these should be stated at the beginning; they are the same as in wilcox.test.}
The option 
...
@
{\bf present the arguments in order.}
...
The group specific intra-cluster correlation $\rho_g$ is controlled by
...
individual level.
The function returns a data set with three columns: 
observed measurement \code{score}, 
grouping id \code{grp}, and cluster id \code{id}.

{\bf always have spell-checker turned on}

As an illustration, a balanced data set contains $10$ cluster of size
5 is generated with treatment assigned at the cluster level:
<<dat-clus>>=
set.seed(616)
...
cluswilcox.test(score ~ grp + cluster(id), data = dat.cls, method = "rgl", 
                exact = TRUE)
@ 
{\bf comments on the results}


The DS test is performed with:
<<clus-rs-ds>>=
cluswilcox.test(score ~ grp + cluster(id), data = dat.cls, method = "ds")
@ 
{\bf comment on the results, compare with those from RGL.
  The statistic are of different signs. Any explanation?}
...
@ 
{\bf comment on the results. different sign of statistics, why?}
...
{\bf why a different default rho from the other function?}
...
generated
{\bf what's so special about 616?}
<<dat-sr>>=
set.seed(616)
...
@
{\bf explain what this function does.}


For example, {\bf use simpower function to show how comparison in one
  scenario can be done before presenting the table.}

{\bf Table~\ref{tab:rs}; just like Section~2, Figure~2, or Equation~2}
Table~\ref{tab:rs} includes results from $10$ scenarios to compare
...
{\bf the table left header needs to be prettified}

{\bf what is delta for the power study?}

{\bf never use h in table positioning, always tbp.}

{\bf use cmidrule in tables}

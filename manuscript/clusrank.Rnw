...
<<simpower>>=
simpower <- function(nrep, level, paired, delta, rho, cor,
                     rate, nclus, maxclsize, ...) {
    do1rep <- function() {
...
replications. The level is set as $0.05$. The true group difference is
$0$ and $0.5$, respectively. The data set contains $10$ clusters, each
with size $3$. The intra-cluster correlation parameter \code{rho} is
0.1. The unit of randomization is cluster.

<<sim-clus-rs>>=
simpower(1000, 0.05, FALSE, 0, c(0.1, 0.1), xch, 1, 10, 5, "cluster")
simpower(1000, 0.05, FALSE, 0.5, c(0.1, 0.1), xch, 1, 10, 5, "cluster")
@ 

...
value of \code{paired}.


To illustrate the usage of the function, we compare the empirical size
and power of the two clustered rank-sum tests through $1000$
replications. The significance level is set as $0.05$. The true group
differences are $0$ and $0.5$, respectively. Each data set contains
$10$ clusters with size $3$. The intra-cluster correlation structure
is exchangeable with parameter \code{rho} equals 0.1. The unit of
randomization is cluster.
{\bf what is the last argument? it should be named}
<<sim-clus-rs>>=
simpower(1000, 0.05, FALSE, 10, 5, 0.0, c(0.1, 0.1), ex, 0, TRUE)
@ 

{\bf say something here. why rho is length 1 here?}
<<sim-sr>>=
simpower(1000, 0.05, TRUE, 10, 5, 0.5, 0.1, ex, 0)
@ 

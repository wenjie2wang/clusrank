...
is set by \codedelta} 
Finally the \code{grplevel} is a switch of the level on
which the treatment is assigned, if \code{grplevel = TRUE}, the treatment 
is assigned at cluster level, otherwise, is is assigned at individual level.

The first dataset has 10 clusters in

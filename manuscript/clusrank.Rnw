...
form a pseudo-sample, and then calculating the Wilcoxon rank-sum
statistic for the pseudo-sample,
\begin{equation*}
  W^* = \frac{1}{N+1}\sum^N_{i = 1}\delta_i^*R_i^*,
  \end{equation*}
where $\delta_i^*$ is the indicator of whether $X_I^*$ is in group~1
and $R_i^*$ is the rank of $X_i^*$ among the pseudo-sample. 
By averaging $W^*$ over all possible pseudo-samples given $\{X_{ij},
\delta_{ij}\}$, 
\begin{equation}\label{eq:dsrsS}
  S = \E(W^* | \mathbf{X}, \mathbf{\delta}),
\end{equation}
FIXME: define notations!  
a large sample test statistic is constructed as
\begin{equation*}
  Z_{\mathrm{DS}} = \frac{S - E(\S)}{\sqrt{\hat{\VAR}(S)}},
\end{equation*}
FIXME: define.
\red{Discuss the conditions of the theoretical results}
...
test for DS rank sum statistic. \red{why not?}
In addition, it cannot be applied to

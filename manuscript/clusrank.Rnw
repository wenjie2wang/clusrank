...
\Abstract{
Wilcoxon rank sum test and signed rank test are widely used
nonparametric tests for two-sample comparison when the distributions
are far from normal or the analyst is unwilling to assume normality. 
The independence assumption of individual data, however, does not hold
for clustered data which are increasingly popular in many fields.
For instance, in ophthalmologic clinical trials, data collected from
the two eyes from the same patient are dependent.
To account for the clustering effect, modifications of the
rank-based tests have been proposed in the literature 
\citep{Rosn:GLyn:Lee:inco:2003, Roxn:Glyn:Lee:wilc:2006}, but no
implementation has been available in \proglang{R}, impeding their
adoption in statistical practice with clustered data.
We introduce an \proglang{R} package \pkg{clusrank}, which provides
Wilcoxon rank sum test and signed rank test for clustered data.
The tests can handle unblanced cluster sizes, presence of ties,
and stratified data, based on the asymptotics for large samples or
...
The Wilcoxon rank sum test and signed rank test are popular
approaches to compare two independent (dependent) groups when the
distribution of each group is unknown or non-normal. To apply the
standard Wilcoxon test, all the observations under study should be
independent within each group. However, in many studies, the
observations are correlated within clusters due to the design of
experiment or by nature. For example, when the responses of indivial
eyes are recorded in ophthalmologic studies, the responses of the eyes
from the same person are naturally positively correlated. Ignoring the
cluster effect usually underestimate the standard error of the test
statistic and hence leads to anticonservative conclusions.


To account for the cluster effect, Wilcoxon tests which have the
cluster effect incorporated have been developed in recent years. To
compare the distribution of two independent groups, a Mann-Whitney U
statistic (which is equivalent to the Wilcoxon rank-sum test) was
proposed by \cite{Rosn:Grov:use:1999} using a corrected variance
estimate which was based on four estimated correlation parameters. But
the large sample theory was not considered and the computation can be
intensive in the presnece of many tied rankings. An alternative
large-sample rank sum test was presented by
\citet{Rosn:Glyn:Lee:inco:2003}. This approach allows to control
covariates by stratification of the data by the covariates. However,
this approach requires the cluster member cannot belong to different
groups. More recently, \citet{Datt:Satt:rank:2005} proposed another
rank-sum statistic which allows clusters to contain members from both
groups and also when the correlation between cluster members differs
across groups. However, all of the aforementioned tests requires the
scales and/or shapes of the distributions to be the same under the
null hypothesis. A weighted Mann-Whitney test for clusted data was
described by \citet{Deni:etal:two:2010} to attack the Beherens-Fisher
problem. This test also allows cluster members from both groups.

For the one-sample problem (for univariate data), the Wilcoxon
signed-rank test was extended to cluster data in \citet{The Wilcoxon
Signed Rank Test for Paired Comparisons of Clustered Data}. To account for the cluster effect, Wilcoxon tests which have the
cluster effect incorporated have been developed. A Mann-Whitney U
statistic(which is equivalent to the Wilcoxon rank-sum test) was
proposed by \cite{Rosn:Grov:use:1999}. However, this approach requires
that identical average rank regardless of the cluster
size, which may not be warrented when the cluster sizes are
unequal. Also, the large sample theory was not considered. An
alternative rank sum test for large sample was considered by
\citet{Rosn:Glyn:Lee:inco:2003}. This approach does not need 
...
\textbf{Need to get right to the point.}




Recent developments have extended the Wilcoxon rank-sum test 
and signed rank test to clustered data.
\textbf{need to have a flow of reviewing the literature. The most
  recent ones should include critics of earlier ones, so we can borrow
  ideas from these papers.}
The rank-sum test is the rank-based version of the two-sample $t$ test.
\citet{Rosn:GLyn:Lee:inco:2003}
proposed a Wilcoxon rank sum test for clustered data
to compare two groups
assuming that the members within each cluster is
\textbf{open spell-check}
exchangeable and objects from the same cluster
...
Another modified
\textbf{See their JASA paper for critics for the RGL method.}
...
Wilcoxon signed-rank test is the nonparametric counterpart of the
paired $t$ test to compare the distributions of paired observations. 
\citet{Laro:wilc:2005} proposed an adjusted version
of wilcoxon signed rank test where the variance estimate is based on
certain sum of square over independent clusters, but this test procedure
is not distribution free \textbf{reference for this claim?}.
\citet{Roxn:Glyn:Lee:wilc:2006} proposed another modified signed rank
test assuming a common intercluster correlation and estimating it
from absolute rank of observations.
\citet{Datt:Satt:sign:2008} proposed a signed rank test procedure 
based on the principle of general within-cluster resampling.
Their test can handle the case when the cluster size is informative.
That is, the distribution of pairwise differences within a cluster 
depends on the cluster size.


Despite the popularity of clustered data in a wide range of applications
such as clinical trials, longitudinal study, social science, and
others, and despite the availability of the rank-based tests for
clustered data, we are unaware of any existing \proglang{R} package
that provides functions implementing the methods reviewed above.
This motivated our development of package \pkg{clusrank} for
...
Section~\ref{sec:test} introduces Wilcoxon rank
sum test and Wilcoxon signed rank test
for clustered data, when data is balanced
or unbalanced. 
\textbf{Do not repeatedly use any expression, e.g., ``in addition''.}
The usage of the main functions are explained in Section~\ref{sec:usage}.
...
\textbf{Put the methods in Section 2;
  put the usages (explain the arguments) in Section 3;
  in Section 4: can add a simulation study to compare the power/size
  (reproduce a table in JASA, e.g.); 
  in Section 5: real data illustration.
  Section 6: discussion for possible extensions.}

\textbf{the name sedlab is a poor choice.}


\section{Rank-Based Tests for Clustered Data}
\label{sec:test} 
...
\label{rank} % DO NOT define if it is not used.

...
the same purpose.
\red{Don't follow this sentence. What is the same purpose?}
...
This means that the RGL method is robust to the violation of the
...
\begin{table}[tbp]
\centering
\caption{Empirical rejection percentage of the RGL method and the DS
  method for rank-sum tests at nominal significance level 0.05.
...
cluster & ex  & 0 & 2 & 0.1, 0.1 & 4.3 & 4.7 & 17.5 & 17.9 & 64.3 & 65.3 \\ 
 ...
 & ar1 & 0 & 2 & 0.1, 0.1 & 4.0 & 4.2 & 18.4 & 19.0 & 65.4 & 66.4 \\ 
  ...
  individual  & ex  & 0 & 2 & 0.1, 0.1 & 4.0 & 4.3 & 17.5 & 16.7 & 69.2 & 67.8 \\ 
...
Comparison between the RGL method and the DS method for the rank-sum test
has not been done when treatment group is assigned at the individual level. 
Nor has it when the intracluster dependence is not exchangeable.
Consider two groups of 20~clusters each, with either exchangeable or AR1
...
balanced data and unbalanced data with missing completely at random. 
...
For each setting, empirical rejection was obtained from 1000 replicates. 
...
of the treatment group level and the intracluster configurations. 
...
the RGL method has higher power with individual-level group assignment. 
...
when the treatment group is assigned at the individual level.
...
\begin{table}[tbp]
  \centering
   \caption{ \red{Mimic the caption and header in Table 1. Captions need to be
       complete, grammaly correct sentences.}
     Empirical rejection percentage comparisons for RGL and DS clustered signed-rank test with exchangeable and ar1 intra-cluster correlation, nominal level $5\%$, $N = 40$ clusters, $1000$ simulations for each combination.}
...
For the signed-rank test, the \citet{Datt:Satt:sign:2008} did not have
...
The intracluster correlation parameter was set to be 0.2, 0.5, and 0.8.
The results are summarized in Table~\ref{tab:sr}.

\red{discuss the results}
...
their counterparts, the Wilcoxon tests for for independent data. 
...
the DS method cannot be applied to collateral data (\red{define?}).


%% \red{Each statement like these should have references; we need to
%% talk on these.}
...
%% derived. 
%%  The rgl method works better when the intra-cluster correlation is 
...
Some other rank-based tests can be incorporated in future developments.
For example, 
\red{list those methods and references that may be implemented 
  in the future.}

...
\code{wilcox.test}. The random permutation test (\code{exact = TRUE},
...
grouping and the RGL clustered signed-rank test (\code{exact = TRUE},
and \code{B = 0}); otherwise \code{exact} should be set as \code{FALSE}.
...
Selected results are summarized in Table~\ref{tab:rsex} and Table~\ref{tab:rsar}.

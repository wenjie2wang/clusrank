...
\proglang{R} package provides easy access to them at one place.
We present a package \pkg{clusrank} where the latest developments 
are implemented and wrapped under a unified user-friendly function.
Different methods are dispatched based on the input, offering great
flexibility in handling various situations such as stratified data.
Exact tests based on permutations are provided for some methods.
Details of the major methods are briefly reviewed.
Usages of the main function are illustrated with simulated data.
Comparison of selected methods that have not been studied before
...
modeling strategy that accounts for intracluster dependence,
...
within-cluster resampling \citep{Hoff:Sen:Weib:with:2001} and 
remains valid when the cluster sizes are informative.
...
More recently, \citet{Dutt:Datt:rank:2015} extended the idea of
within-cluster resampling to further accommodate the case where the
number of subjects in a group within a cluster is informative.
...
Package \pkg{clusrank} made first CRAN appearance in December, 2015.
The package provides these recently available rank-sum tests
...
Before package \pkg{clusrank} appeared, the RGL methods were 
...
\proglang{R} codes for the DL methods were available from Dr. Datta's website
(\url{http://www.somnathdatta.org/software/}), which was recently
put into \proglang{R} package \pkg{ClusterRankTest} in April, 2016.
Modeled after the familiar \code{wilcox.test}, package \pkg{clusrank}
unifies both RGL methods and DS methods under a unified, user-friendly
...
different approaches under various settings; see Section~\ref{s:simu}.
...
The usage of the unified user-level function and major input arguments
are described in Section~\ref{s:usage}, and illustrations of how to
access the implemented methods are presented in Section~\ref{s:illus}.
In Section~\ref{s:simu}, comparisons of selected methods in some 
settings that have not been studied previously are reported, which
are made very easy by the package and a few lines of code.
...
The maximum cluster size $M = \max_i n_i$ has three levels $2$, $5$ and $10$. 
...
\begin{table}[tbp]
  \centering
  \caption{Empirical rejection percentage  comparisons for RGL and DS
    rank-sum test with cluster-level and individual-level grouping, nominal level $5\%$, $N = 40$ clusters, $M$ is the maximum cluster size, $1000$ simulations for each combination.}
...
Group. &  Corr. str. & Missing rate & M &   $\rho$ & RGL & DS & RGL & DS & RGL & DS \\

  \midrule
  
clus & ex  & 0 & 2 & 0.1, 0.1 & 4.3 & 4.7 & 17.5 & 17.9 & 64.3 & 65.3 \\ 
 ...
 & ar1 & 0 & 2 & 0.1, 0.1 & 4.0 & 4.2 & 18.4 & 19.0 & 65.4 & 66.4 \\ 
  ...
  ind  & ex  & 0 & 2 & 0.1, 0.1 & 4.0 & 4.3 & 17.5 & 16.7 & 69.2 & 67.8 \\ 
...
As shown in table~\ref{tab:rs}, the empirical size of both methods are
close to the nominal level $0.05$. When the cluster size is assigned
at cluster level,  the DS method outperforms the RGL method in most
cases, which might because that RGL method represents each cluster
with its rank-sum whereas the DS method exerts more information in
nature by represneting each cluster with its conditional expected rank
of a randomly picked member. In addition, the size of the RGL method
is robust to the violation of the assumption of exchangeability of
intra-cluster correlation when the correlation structure is AR1. 
The balancedness of the data set has little impact on the power of
both methods since with an average cluster size equals $5$, the power
of the balanced and unbalanced are comparable.


In contrast to the cluster-level grouping data, the RGL method
performs better when treatment is assigned to each single individual
in most cases under our experimental conditions. This is because that
the RGL method considers the group comparison within each cluster
whereas the DS method does not. This can be further illustrated in the
case when there is less within cluster comparison, e.g., when cluster
size is $2$, the performance of the two methods are then
comparable. 
Another interesting point is that, unlike the cluster-level grouping
data, the power of both tests tends to increase as the correlation
increases for individual-level grouping data, since the comparison of
different treatment on similar objects will highlight the treatment
effect. 


\subsection{Signed-rank tests}
For clustered signed-rank tests, we generate data using
\code{datgen.sr}. 
...
\begin{tabular}{rrrrrrrrrr}
  \toprule
  &&&& \multicolumn{2}{c}{$\delta = 0$} & \multicolumn{2}{c}{$\delta = 0.1$} & \multicolumn{2}{c}{$\delta = 0.3$} \\
  \cmidrule(lr){5-6} \cmidrule(lr){7-8} \cmidrule(lr){9-10}
  Corr. str. & Missing rate & M &   $\rho$ & RGL & DS & RGL & DS & RGL & DS \\
  \midrule
 ex & 0  & 2 &0.1, 0.1 &  6.1 & 6.2 & 13.6 & 13.7 & 68.9 & 69.2 \\ 
  &&   &      0.5, 0.5 &  5.0 & 5.0 & 10.0 & 10.4 & 57.1 & 57.4 \\ 
  &&  &      -0.1, 0.9 &  5.2 & 5.4 & 10.6 & 11.0 & 54.8 & 54.9 \\ 
  &    & 10&  0.1, 0.1 &  6.0 & 6.0 & 40.4 & 40.2 & 99.9 & 99.9 \\ 
  &&   &      0.5, 0.5 &  4.5 & 4.5 & 21.6 & 22.0 & 93.4 & 93.4 \\ 
  &&   &     -0.1, 0.9 &  4.9 & 4.9 & 17.4 & 17.6 & 80.0 & 80.3 \\ 
  &0.5 & 5 & 0.1, 0.1  &  4.1 & 4.6 & 17.5 & 15.4 & 78.4 & 71.4 \\ 
  &&   &      0.5, 0.5 &  4.7 & 5.3 & 12.8 & 13.2 & 65.3 & 62.4 \\ 
  &&   &     -0.1, 0.9 &  4.6 & 4.2 & 12.9 & 13.0 & 57.8 & 57.0 \\ 
  &    & 10 & 0.1, 0.1 &  5.3 & 5.0 & 23.8 & 21.6 & 96.5 & 94.7 \\ 
  &&   &      0.5, 0.5 &  6.1 & 5.9 & 16.5 & 16.6 & 86.4 & 84.3 \\ 
  &&   &     -0.1, 0.9 &  4.9 & 5.0 & 14.1 & 13.8 & 74.6 & 73.6 \\ 
                         
 ar1& 0 & 2 & 0.1, 0.1 & 5.3 & 5.2 & 11.4 & 11.4 & 67.5 & 67.9 \\ 
  &&   &      0.5, 0.5 & 4.4 & 4.5 & 11.2 & 11.6 & 56.3 & 56.5 \\ 
  &&   &     -0.1, 0.9 & 5.1 & 5.2 & 12.0 & 12.2 & 60.6 & 60.8 \\ 
  &    & 10 & 0.1, 0.1 & 5.4 & 5.4 & 38.4 & 38.5 & 100.0 & 99.9 \\ 
  &&   &      0.5, 0.5 & 4.3 & 4.4 & 21.7 & 21.9 & 94.3 & 94.5 \\  
  &&   &     -0.1, 0.9 & 4.0 & 4.0 & 16.7 & 17.2 & 81.4 & 81.4 \\ 
   &0.5 & 5 & 0.1, 0.1 & 4.2 & 5.0 & 8.5 & 8.8 & 38.4 & 37.3 \\   
   &&   &     0.5, 0.5& 5.9 & 6.0 & 7.1 & 7.5 & 35.2 & 35.7 \\   
   &&   &    -0.1, 0.9& 5.8 & 6.2 & 7.9 & 8.0 & 38.3 & 38.0 \\   
   &    & 10 &0.1, 0.1& 4.8 & 4.9 & 25.2 & 23.4 & 96.3 & 95.6 \\ 
   &&   &     0.5, 0.5& 5.4 & 6.0 & 18.9 & 19.3 & 87.3 & 86.5 \\ 
   &&   &    -0.1, 0.9& 4.3 & 4.5 & 13.7 & 13.7 & 72.5 & 71.9 \\ 
...
Both methods maintain an empirical size close to the nominal size
$0.05$. They also have comparable empirical power in almost all cases
except the RGL method outperforms the DS method when there is
variation within the cluster size, e.g., when we randomly introduce
missingness in the data to construct unbalanced data.
Again, the empirical size of the RGL method is robust to the AR1
correlation. 

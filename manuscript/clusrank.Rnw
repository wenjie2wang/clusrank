...
intracluster dependence have been recently developed, 
but few existing R package provides easy access to them at one place
\citep{Dutt:Datt:clus:2016}.  
...
are easily performed with the package and the results are
discussed. We acknowledge the \proglang{SAS} and \proglang{R} code provided
by the authors of the methods implemented in this package
\citep{SAS:inco:2016, SAS:sub:2016, SAS:sign:2016, R:clus:2016}.
\red{No citations in the abstract please. Reviews should be put in the
  introduction at the right places.}
...
only the DS method of the extensions of Wilcoxon rank-sum and
signed-rank tests reviewed above and another recent developed
clustered rank-sum test \citep{Dutt:Datt:clus:2016} has only been implemented in a
\proglang{R} package \pkg{ClusterRankTest} by the athors
\citep{Datt:Satt:rank:2005, Datt:Satt:sign:2008, Dutt:Datt:clus:2016}.  This motivated our development of
package \pkg{clusrank}.  Under a unified interface similar to
\code{wilcox.test}, the package provides these recently available
rank-sum tests \citep{Rosn:Glyn:Lee:inco:2003,
  Rosn:Glyn:Lee:exte:2006, Datt:Satt:rank:2005} and signed-rank tests
\citep{Rosn:Glyn:Lee:wilc:2006, Datt:Satt:sign:2008} for clustered
data.  The package makes it very easy to compare the performances of
the different approaches under various settings.
...
zero and an exchangeable or a AR1 correlation structure with
correlation parameter $\rho_g$, and $\delta$ is the group difference.
<<corstr>>=
ex  <- function(dim, rho) {
...
sizes. The needed multivariate normal random variable is generated
with \pkg{mvtnorm} \citep{Genz:etal:mvtn:2016, Genz:Bret:comp:2009}.
<<datgen, echo=TRUE>>= 
library(mvtnorm)
datgen.rs <- function(nclus, maxclsize, delta = 0., rho = c(0.1, 0.1), 
                      cor = ex, rate = 0., clusgrp = TRUE) {
    nn <- nclus * maxclsize
    ...
    drop <-  sort(sample(1:(2 * nn), size = rate * (2 * nn), FALSE))
    if (rate == 0.) dat else dat[-drop, ]      
}
@ 
The function have two required inputs: \code{nclus} for number of
clusters in each group and \code{maxclsize} for the maximum cluster
size. 
The difference between the groups is specified by \code{delta}.
The group specific intra-cluster correlation parameter $\rho_g$ is
controlled by \code{rho}, a numeric vector of length 2, one for each
group.  There are two correlation structures to choose from by setting
\code{cor}: \code{ex} for exchangeable correlation and \code{ar1} for
AR1 correlation.  
To allow unequal cluster sizes, \code{rate} specifies the missing rate
at which an individual in a cluster to be excluded at random; when
\code{rate = 1}, balanced data with equal cluster size is generated.  
The grouping level is controlled by the logical \code{clusgrp}:
\code{TRUE} for cluster-level and \code{FALSE} for individual level.
The function returns a data set with three columns: observed
measurement \code{x}, grouping id \code{grp}, and cluster id
\code{cid}. 


For the replicability of the following demonstration,
a random seed is first fixed. 
<<seed>>=
set.seed(1234)
@ 

To illustrate the clustered rank sum test, a data set with $10$
clusters is generated with null treatment effect. 
Each cluster contains $3$ members and the intra-cluster correlation
is exchangeable with $\rho_g = 0.9$. 
The basic unit of the treatment assignment is cluster.

<<dat-clus>>=
dat.cl <- datgen.rs(10, 3, 0, c(.9, .9), ex, 0, TRUE)
head(dat.cl)
@ 

We first apply the formula interface and the RGL asymptotic test to the
data set: 
<<clus-rs-rgl-asymp>>=
clusWilcox.test(x ~ grp + cluster(cid), dat.cl, method = "rgl")
@ 
The permutation test of RGL method is also available for data with a
...
We illustrate the use of numerical interface with DS method in the following:
<<clus-rs-ds>>=
...
For cluster-level grouping data, the RGL method allows an extra
stratum variable to add information to the test. The data set
\code{dat.str} generated below contains a strata variable besides the
required variables. It is obtained by combining two data sets:
\code{dat.cl}, and \code{dat.cl1} which is generated similarly as
\code{dat.cl} but with a non-zero effect equals $0.5$. Each data set
is treated as from a separate stratum, and a \code{strat} variable is
added to the combined data set to identify the strata.


<<clus-rs-rgl-str>>=
dat.cl1 <- datgen.rs(10, 3, 0, c(.9, .9), ex, 0, TRUE)
dat.cl1$cid <- rep(21 : 40, each = 3)
dat.str <- cbind(rbind(dat.cl, dat.cl1), strat = rep(c(1, 2), each = 60))
head(dat.str)
clusWilcox.test(x ~ grp + cluster(cid) + stratum(strat), dat = dat.str, 
                method = "rgl")
@ 


A benefit of the DS method is that it can compare more than two
groups. We construct a data set with $4$ groups by combining
\code{dat.cl} and \code{dat.cl1} but assign a different set of group
id to \code{dat.cl1} than that of \code{dat.cl}.

<<clus-rs-ds-mgrp>>=
dat.cl1$grp <- rep(c(3, 4), each = 30)
dat.m <- rbind(dat.cl, dat.cl1)
clusWilcox.test(x ~ grp + cluster(cid), dat = dat.m, method = "ds")
@ 
...
Note: the DS paper hasn't done unbalanced, non-exchangeable simulation.

We generate the data for clustered signed-rank test similarly as in
simulation scenario $1$ in \citet{Datt:Satt:sign:2008}, both balanced
data and unbalanced data. The observations in a cluster is generated
as
\[
  X = \mathrm{sign}(Z_d)\exp(|Z_d|),
\]
where $Z_d$ is a multivariate normal random vector with mean $\delta$
and an exchangeable or AR1 conrrelation structure with parameter $\rho$. 

<<datagen-sr, echo = TRUE>>=
datgen.sr <- function(nclus, maxclsize, delta = 0.,
                      rho = c(0.1, 0.1), cor = xch, rate = 0) {
    nn <- nclus * maxclsize
    Sigma1 <- cor(maxclsize, rho[1])
    Sigma2 <- cor(maxclsize, rho[2])
...
For the ease of the simulation study in the following section, all of
the arguments of \code{datgen.sr} are inherited from \code{datgen.rs}
as necessary. No grouping is involved in the data generation for the
signed rank test, therefore there is no \code{clusgrp} argument.
In addition, to allow variation in the intra-cluster correlation, 
correlation parameter \code{rho} is a vector of two, each controls
\code{nclus} clusters. 



We illustrate the clustered signed rank test with a data set that
contains $10$ clusters. Each cluster contains $3$ pair differences.
An exchangeble correlation with parameter \code{rho} equals $0.9$
is used as the intra-cluster correlation. 

<<dat-sr>>=
dat.sr <- datgen.sr(10, 3, cor = ex, rho = c(0.1, 0.9))
head(dat.sr)
@ 

To apply both methods to this data set:
<<sr>>=
clusWilcox.test(x ~ cluster(cid),  dat.sr, paired = TRUE, method = "rgl")
clusWilcox.test(x ~ cluster(cid),  dat.sr, paired = TRUE, method = "ds")
@ 

\section{A Simulation Study}
\label{s:simu}

Despite the recent literature, comparisons of the RGL method and the
DS method under certain senarios have not been studied.
Such comparison can be easily done with package \pkg{clusrank}. 
Using the two data generation functions defined above, we report a
simulation study comparing their sizes and powers under a variety of
scenarios. The following function generates replicates of data for a
...
between the clustered signed-rank test and rank-sum test. The value of
\code{delta} determines the group difference. The rest arguments are
inherited from \code{datgen.rs} or \code{datgen.sr} depending on the
value of \code{paired}. When \code{paired} is \code{FALSE}, i.e.,
clustered rank-sum test is under study, the last argument, \code{...},
is the \code{clusgrp} argument in \code{datgen.rs} to determine the
unit of randomization. 


To illustrate the usage of the function, we compare the empirical size
of the two clustered rank-sum tests and the two clustered signed-rank
tests through $1000$ replications on balanced data set. 
The significance level is set as $0.05$. 
The true group differences is $0.$. For clustered rank-sum test each
data set contains $20$ clusters with size $3$ for each group.  
The intra-cluster correlation structure is exchangeable. The
correlation parameter \code{rho} is set as $0.1$ for both groups.
The missing rate is set as $0$ to obtain balanced data.
The unit of randomization is cluster. 
...
For the clustered signed-rank tests, we use the same setting as in the
above simulation for clustered rank-sum test, except that the
correlation structure is now AR1 which is not exchangeable and no
\code{clusgrp} is needed since there is no explicit treatment 
assignment for signed-rank test data.  
<<sim-sr>>=
simpower(1000, 0.05, TRUE, 20, 3, 0., c(0.5, 0.5), ar1, 0)
@ 
Both methods have a empirical size that is close to the nominal level
which shows that even though the RGL method requires the exchangeability in
the derivation of the asymptotic distribution of the test statistic,
it is quite robust to this assumption when the correlation structure
is AR1. 


\subsection{Rank-sum tests}
A relatively comprehensive comparison of the two clustered rank-sum
tests has been done by \citet{Datt:Satt:rank:2005} when treatment is
assigned to clusters. No comparison has been done when the treatment
is assigned at individual level yet.  Table~\ref{tab:rs} shows the
results from $12$ designs using function \code{simpower} to compare
the two clustered rank-sum tests under different conditions when each
individual is the unit of randomization. 
Each data set contains $40$ clusters. Two intra-cluster correlation
structures, exchangeable and autocorrelated with order~1 (ar1), are used, 
each with correlation coefficient $\rho = 0.1$ and $0.9$. 
The maximum cluster size $M = \max_i n_i$ has two levels $5$ and $10$. 
When maximum cluster size is $5$, we compare the
performance of the two tests when data is balanced and unbalanced, by
setting \code{rate} of observations which are randomly kept to be $1$
and $0.5$. In order to introduce more variation in the cluster size of
the unbalanced data while maintaining the same sample size, we
increase $M$ tp $10$ and decrease \code{rate} to $0.25$. 
The group difference $\delta$ takes $0$, $0.3$ and $0.6$ respectively. 
Each cell represents $1000$ replications.

\red{Read the log files and fix the warnings.}

\begin{table}[tbp]
  \centering
  \caption{Empirical rejection percentage  comparisons for RGL and DS clustered rank-sum test with cluster level grouping, nominal level $5\%$, $N = 40$ clusters, $M$ is the maximum cluster size, $1000$ simulations for each combination.}
\begin{tabular}{rrrrrrrrrr}
  \toprule
  &&&& \multicolumn{2}{c}{$\delta = 0$} & \multicolumn{2}{c}{$\delta = 0.2$} & \multicolumn{2}{c}{$\delta = 0.5$} \\
  \cmidrule(lr){5-6} \cmidrule(lr){7-8} \cmidrule(lr){9-10}
  Corr. str. & Missing rate & M &   $\rho$ & RGL & DS & RGL & DS & RGL & DS \\

  \midrule
  
  ex  & 0 & 2 & 0.1, 0.1 & 4.3 & 4.7 & 17.5 & 17.9 & 64.3 & 65.3 \\ 
  &&&           0.5, 0.5 & 3.5 & 3.7 & 14.2 & 14.8 & 53.2 & 54.5 \\ 
  &&&          -0.1, 0.9 & 3.4 & 3.6 & 14.1 & 14.8 & 59.5 & 60.5 \\ 
  &&       10 & 0.1, 0.1 & 4.2 & 4.4 & 41.4 & 42.5 & 97.4 & 97.6 \\ 
  &&&           0.5, 0.5 & 4.0 & 4.2 & 19.0 & 19.9 & 63.6 & 64.8 \\ 
  &&&          -0.1, 0.9 & 6.1 & 6.4 & 15.8 & 16.9 & 90.2 & 90.5 \\ 
  &   0.5 & 2 & 0.1, 0.1 & 4.6 & 4.9 & 7.8 & 10.8 & 40.8 & 41.4 \\ 
  &&&           0.5, 0.5 & 5.3 & 5.5 & 11.6 & 11.2 & 29.9 & 32.9 \\ 
  &&&          -0.1, 0.9 & 5.0 & 5.8 & 9.3 & 10.5 & 35.9 & 36.7 \\ 
  &&       10 & 0.1, 0.1 & 5.9 & 6.1 & 26.0 & 30.3 & 85.4 & 89.1 \\ 
  ...
  &&       10 & 0.1, 0.1 & 4.1 & 4.4 & 61.0 & 62.4 & 99.8 & 99.8 \\ 
  &&&           0.5, 0.5 & 4.8 & 5.1 & 32.5 & 33.8 & 92.5 & 93.2 \\ 
  &&&          -0.1, 0.9 & 5.1 & 5.3 & 19.8 & 21.1 & 89.8 & 90.6 \\ 
  &   0.5 & 2 & 0.1, 0.1 & 5.4 & 6.4 & 11.3 & 11.5 & 35.6 & 36.8 \\ 
  &&&           0.5, 0.5 & 5.1 & 5.6 & 10.5 & 10.4 & 31.4 & 35.0 \\ 
  &&&          -0.1, 0.9 & 6.2 & 7.2 & 8.1 & 9.6 & 35.2 & 36.9 \\ 
  &&       10 & 0.1, 0.1 & 5.7 & 6.6 & 28.3 & 32.2 & 90.3 & 93.9 \\ 
  &&&           0.5, 0.5 & 4.6 & 4.9 & 22.3 & 26.0 & 76.9 & 84.6 \\ 
  &&&          -0.1, 0.9 & 4.5 & 5.1 & 15.5 & 20.3 & 68.7 & 77.8 \\ 
   \bottomrule
\end{tabular}
\label{tab:rsclus}
\end{table}

\red{If you don't separate them by blank lines, they will be treated as
  one paragraph!}
% latex table generated in R 3.3.1 by xtable 1.8-2 package
% Tue Aug 16 21:57:22 2016
\begin{table}[tbp]
\centering
  \caption{Empirical rejection percentage  comparisons for RGL and DS clustered rank-sum test with individual level grouping, nominal level $5\%$, $N = 40$ clusters, $M$ is the maximum cluster size, $1000$ simulations for each combination.}
\begin{tabular}{rrrrrrrrrr}
  \toprule
  &&&& \multicolumn{2}{c}{$\delta = 0$} & \multicolumn{2}{c}{$\delta = 0.2$} & \multicolumn{2}{c}{$\delta = 0.5$} \\
  \cmidrule(lr){5-6} \cmidrule(lr){7-8} \cmidrule(lr){9-10}
  ...
  \midrule
 ex  & 0 & 2 & 0.1, 0.1 & 4.0 & 4.3 & 17.5 & 16.7 & 69.2 & 67.8 \\ 
 &&&           0.5, 0.5 & 4.6 & 5.0 & 19.8 & 19.6 & 69.1 & 68.0 \\ 
 &&&          -0.1, 0.9 & 4.9 & 5.0 & 18.0 & 16.9 & 69.2 & 68.6 \\ 
 &&       10 & 0.1, 0.1 & 5.3 & 4.5 & 65.1 & 63.2 & 100.0 & 100.0 \\ 
 &&&           0.5, 0.5 & 4.7 & 5.0 & 70.1 & 66.1 & 99.8 & 99.7 \\ 
 &&&          -0.1, 0.9 & 4.3 & 4.2 & 70.7 & 65.5 & 99.8 & 99.8 \\ 
 &   0.5 & 2 & 0.1, 0.1 & 4.9 & 5.5 & 13.3 & 11.8 & 40.5 & 37.9 \\ 
 &&&           0.5, 0.5 & 4.0 & 3.9 & 12.5 & 12.2 & 40.5 & 36.6 \\ 
 &&&          -0.1, 0.9 & 5.5 & 5.9 & 12.1 & 12.8 & 40.1 & 36.6 \\ 
 &&       10 & 0.1, 0.1 & 5.5 & 4.9 & 40.4 & 38.9 & 97.2 & 94.9 \\ 
 ...
 &&       10 & 0.1, 0.1 & 5.1 & 5.3 & 68.3 & 64.7 & 100.0 & 100.0 \\ 
 &&&           0.5, 0.5 & 6.1 & 6.1 & 69.5 & 66.5 & 100.0 & 100.0 \\ 
 &&&          -0.1, 0.9 & 4.7 & 5.2 & 67.9 & 63.9 & 99.9 & 100.0 \\ 
 &   0.5 & 2 & 0.1, 0.1 & 5.9 & 4.8 & 11.4 & 13.3 & 42.3 & 40.2 \\ 
 &&&           0.5, 0.5 & 5.1 & 6.5 & 11.5 & 11.8 & 41.0 & 38.9 \\ 
 &&&          -0.1, 0.9 & 4.8 & 6.0 & 10.2 & 10.2 & 39.4 & 37.0 \\ 
 &&       10 & 0.1, 0.1 & 5.1 & 5.3 & 37.5 & 34.4 & 96.1 & 94.1 \\ 
 &&&           0.5, 0.5 & 4.2 & 4.5 & 39.1 & 35.6 & 96.7 & 93.1 \\ 
 &&&          -0.1, 0.9 & 5.1 & 4.8 & 43.0 & 35.0 & 97.5 & 94.0 \\ 
 \bottomrule
\end{tabular}
\label{tab:rsind}
\end{table}

\red{highlight your discussion on the comparison that has not been done.
  what do these results suggest? are they as expected?}


The empirical size of both methods are around the nominal level in all
scenarios, though the performance in terms of power differs. When the
intra-cluster correlation is high, the RGL method is more competent
than DS method regardless of other configurations, while their
performance is comparable when the correlation is low. 
...
\subsection{Signed-rank tests}
For clustered signed-rank tests, we compare the RGL and DS methods for
data with cluster level and individual grouping respectively.
Each generated data set contains $40$ clusters with
a maximum cluster size $M = 2$ and $10$. Two correlation structures,
exchangeable and AR1, are used.
We also compared the two tests with balanced and unbalanced data by
setting missing rate as $0$ and $0.5$. There are $1000$ replications
for each design.

\red{these two tables can be combined into one}
\begin{table}[tbp]
  \centering
   \caption{Empirical rejection percentage comparisons for RGL and DS clustered signed-rank test with exchangeable intra-cluster correlation, nominal level $5\%$, $N = 40$ clusters, $1000$ simulations for each combination.}
\begin{tabular}{rrrrrrrrr}
  \toprule
   &&& \multicolumn{2}{c}{$\delta = 0$} & \multicolumn{2}{c}{$\delta = 0.2$} & \multicolumn{2}{c}{$\delta = 0.5$} \\
  \cmidrule(lr){4-5} \cmidrule(lr){6-7} \cmidrule(lr){8-9}
  Missing rate & M &   $\rho$ & RGL & DS & RGL & DS & RGL & DS \\
  \midrule
  0   & 2 &  0.1, 0.1 & 5.2 & 5.4 & 37.7 & 38.3 & 98.0 & 98.0 \\ 
  &   &      0.5, 0.5 & 5.0 & 5.2 & 29.1 & 29.4 & 93.3 & 93.5 \\ 
  &   &      0.9, 0.9 & 5.4 & 5.5 & 23.5 & 23.7 & 86.6 & 87.0 \\ 
  &   &     -0.1, 0.9 & 3.7 & 3.9 & 30.1 & 30.6 & 95.9 & 96.0 \\ 
      & 10 & 0.1, 0.1 & 5.8 & 5.8 & 80.5 & 80.8 & 100.0 & 100.0 \\ 
  &   &      0.5, 0.5 & 5.6 & 5.6 & 39.2 & 39.7 & 98.6 & 98.6 \\ 
  &   &      0.9, 0.9 & 5.3 & 5.4 & 24.4 & 24.8 & 87.5 & 87.5 \\ 
  &   &     -0.1, 0.9 & 5.6 & 6.0 & 42.3 & 42.4 & 98.4 & 98.4 \\ 
  0.5 & 2 &  0.1, 0.1 & 3.7 & 5.4 & 22.0 & 21.6 & 81.6 & 80.5 \\ 
  &   &      0.5, 0.5 & 4.7 & 4.4 & 18.4 & 18.7 & 77.9 & 77.6 \\ 
  &   &      0.9, 0.9 & 5.2 & 5.3 & 18.9 & 18.9 & 73.9 & 74.0 \\ 
  &   &     -0.1, 0.9 & 6.0 & 6.6 & 20.6 & 21.5 & 78.6 & 77.7 \\ 
      & 10 & 0.1, 0.1 & 5.2 & 5.3 & 58.3 & 56.6 & 100.0 & 100.0 \\ 
  &   &      0.5, 0.5 & 5.2 & 4.7 & 35.5 & 35.4 & 96.9 & 97.2 \\ 
  &   &      0.9, 0.9 & 5.5 & 5.6 & 25.3 & 25.7 & 86.8 & 87.1 \\ 
  &   &     -0.1, 0.9 & 4.5 & 4.6 & 40.3 & 40.0 & 98.4 & 98.4 \\ 
\bottomrule
\end{tabular}
\label{tab:srxch}
\end{table}
...
\begin{table}[tbp]
  \centering
   \caption{Empirical rejection percentage comparisons for RGL and DS clustered signed-rank test with AR1 intra-cluster correlation, nominal level $5\%$, $N = 40$ clusters, $1000$ simulations for each combination.}
\begin{tabular}{rrrrrrrrr}
  \toprule
   &&& \multicolumn{2}{c}{$\delta = 0$} & \multicolumn{2}{c}{$\delta = 0.2$} & \multicolumn{2}{c}{$\delta = 0.5$} \\
  \cmidrule(lr){4-5} \cmidrule(lr){6-7} \cmidrule(lr){8-9}
  Missing rate & M &   $\rho$ & RGL & DS & RGL & DS & RGL & DS \\
  \midrule
  0   & 2 &  0.1, 0.1  & 5.0 & 5.1 & 35.2 & 35.9 & 97.7 & 97.7 \\ 
  &   &      0.5, 0.5  & 5.8 & 5.8 & 31.6 & 32.0 & 93.8 & 93.9 \\ 
  &   &      0.9, 0.9  & 5.4 & 5.7 & 21.6 & 21.9 & 86.8 & 86.9 \\ 
  &   &     -0.1, 0.9  & 4.3 & 4.5 & 30.0 & 30.1 & 94.0 & 94.1 \\ 
      & 10 & 0.1, 0.1  & 5.1 & 5.3 & 92.7 & 92.4 & 100.0 & 100.0 \\ 
  &   &      0.5, 0.5  & 4.4 & 4.3 & 68.1 & 68.0 & 100.0 & 100.0 \\ 
  &   &      0.9, 0.9  & 4.4 & 4.7 & 30.1 & 30.3 & 92.5 & 92.8 \\ 
  &   &     -0.1, 0.9  & 5.3 & 5.3 & 50.9 & 51.4 & 99.5 & 99.5 \\ 
  0.5 & 2 &  0.1, 0.1  & 4.8 & 5.2 & 21.0 & 20.4 & 84.9 & 82.0 \\ 
  &   &      0.5, 0.5  & 4.5 & 4.1 & 19.4 & 19.1 & 77.8 & 76.6 \\ 
  &   &      0.9, 0.9  & 4.9 & 4.9 & 18.5 & 19.3 & 73.2 & 73.6 \\ 
  &   &     -0.1, 0.9  & 4.7 & 4.4 & 19.1 & 19.1 & 79.0 & 78.9 \\ 
      & 10 & 0.1, 0.1  & 5.2 & 5.0 & 72.9 & 67.8 & 100.0 & 100.0 \\ 
  &   &      0.5, 0.5  & 5.4 & 5.6 & 52.1 & 50.5 & 99.9 & 99.9 \\ 
  &   &      0.9, 0.9  & 5.1 & 5.7 & 26.7 & 27.2 & 93.8 & 94.2 \\ 
  &   &     -0.1, 0.9  & 4.4 & 4.2 & 45.8 & 44.0 & 98.9 & 98.9 \\ 
\bottomrule
\end{tabular}
\label{tab:srar1}
\end{table}

The two methods are comparable under all designs and both methods
maintain an empirical size close to the nominal size $0.05$. The two correlation
structure have little effect on the empirical power of both tests. An
increase in the correlation magnitude, though, will reduce the
empirical power by between a third and a half as when $\delta = 0.3$. 
...
\red{discuss those methods and references that may be implemented 
  in the future.}

...
\pkg{stats} of \proglang{R} and \code{PROC NPAR1WAY} of \proglang{SAS}.
...
When the treatments are permutated within each cluster, for instance in a
multi-center randomized clinical trial where randomization occurs within
each center, the \proglang{R} R package \pkg{coin}
\citep{Hoth:etal:impl:2008} is a good tool kit of conditional
permutation test controlled on the blocks. However, the situation where the
sampling unit is cluster or cluster along with its subunits is not under the
framework of the package \pkg{coin}.


One common practice to extend Wilcoxon rank-sum test to clustered data is to treat
clusters as independent individuals and use the mid-rank of the subunits as
the "rank" of each cluster and apply the ordinary Wilcoxon rank-sum test
(e.g., as implemented in \proglang{StatXact} \citep{Cyte:stat:2013}).
...
and in \proglang{STATA} \citep{stat:2015} package \pkg{cluswilcox} from Dr. Lee's website
(\url{http://cls.umd.edu/mtlee/}).

...
\code{subset} and \code{na.action} are used the same as the usage of
them in the \code{model.frame} function to subset the data and
determine the action to \code{NA} values in the data. The other
arguments are identical to those in the default interface.

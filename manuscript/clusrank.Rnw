...
\section{Usage}
The single interface function \code{cluswilcox.test} in the package
resembles the function \code{wilcox.test} in the \pkg{stats} package
which is shared by the ranksum test and signed rnak test.

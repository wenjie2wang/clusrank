...
sizes, \pkg{mvtnorm} \citep{Genz:etal:mvtn:2016, Genz:Bret:comp:2009}
is applied to generate the needed multivariate
normal random variable, and \pkg{binarySimCLF}
\citep{By:Qaqi:bina:2009} is used to generate exchangeble and AR1
...
There are two correlation structures to choose from by setting
...
\code{dat.cls} but with a non-zero effect equals $0.5$. Each data set is treated as from a separate stratum, and a \code{strat} variable is added to the combined data set to identify the strata.


<<clus-rs-rgl-str>>=
dat.cls1 <- datgen.rs(10, 3, 0.5, c(.9, .9), xch,  1, "cluster")
dat.str <- cbind(rbind(dat.cls1), strat = rep(c(1, 2), each = 30))
head(dat.str)
clusWilcox.test(score ~ grp + cluster(cid) + stratum(strat), 
                dat = dat.str,  method = "rgl")
@ 


A benefit of the DS method is that it can compare more than two
groups. We construct a data set with $4$ groups by combining
\code{dat.cls} and \code{dat.cls1} but assign a different set of group id to \code{dat.cls1} than that of \code{dat.cls}. 

<<clus-rs-ds-mgrp>>=
dat.cls1$grp <- rep(c(3, 4), each = 15)
...
We generate the data for clustered signed-rank test similarly as in simulation scenario $1$ in \citet{Datt:Satt:sign:2008}, both balanced data and unbalanced data. The observations in a cluster is generated as 
\[
  X = sign(Z_d)\exp(|Z_d|),
\]
where $Z_d$ is a multivariate normal random vector with mean $\delta$ and an exchangeable or AR1 conrrelation structure with correlation parameter $\rho$, and $\delta$ is the pair difference.

...
datgen.rs <- function(nclus, maxclsize, delta = 0., rho = c(0.1, 0.1), 
                      cor = c(ex, ar1), rate = 1, clusgrp = TRUE) {
    nn <- nclus * maxclsize
...
size.  The difference between the groups is specified by \code{delta}.
...
\code{cor}: \code{xch} for exchangeable correlation and \code{ar1} for
AR1 correlation.  To allow unequal cluster sizes, \code{rate}
specifies the rate at which an individual in a cluster is kept at
random; when \code{rate = 1}, balanced data with equal cluster size is
generated.  The grouping level is controlled by \code{grplevel}:
\code{"cluster"} for cluster-level and \code{"individual"} for
individual level.  The function returns a data set with three columns:
observed measurement \code{x}, grouping id \code{grp}, and cluster
id \code{cid}.
...
clusters is generated with treatment effect null. 
Each cluster has $3$ members and an exchangeable correlation with
$\rho_g = 0.9$. The basic unit of the treatment assignment is cluster.

<<dat-clus>>=
dat.cls <- datgen.rs(10, 3, 0, c(.9, .9), xch,  1, "cluster")
head(dat.cls)
@ 
...
data set: 
<<clus-rs-rgl-asymp>>=
clusWilcox.test(x ~ grp + cluster(cid), dat.cls, method = "rgl")
@ 
...
<<clus-rs-rgl-exact>>=
clusWilcox.test(x ~ grp + cluster(cid), dat.cls, method = "rgl", exact = TRUE)
@ 


We illustrate the use of numerical interface with DS method in the following:
<<clus-rs-ds>>=
clusWilcox.test(x, group =  grp, cluster = cid, data = dat.cls, method = "ds")
@ 
...
\code{dat.cls}, and \code{dat.cls1} which is generated similarly as
\code{dat.cls} but with a non-zero effect equals $0.5$. Each data set
is treated as from a separate stratum, and a \code{strat} variable is
added to the combined data set to identify the strata.


<<clus-rs-rgl-str>>=
dat.cls1 <- datgen.rs(10, 3, 0, c(.9, .9), xch,  1, "cluster")
dat.cls1$cid <- rep(21:40, each = 3)
dat.str <- cbind(rbind(dat.cls, dat.cls1), strat = rep(c(1, 2), each = 60))
head(dat.str)
...
\code{dat.cls} and \code{dat.cls1} but assign a different set of group
id to \code{dat.cls1} than that of \code{dat.cls}.

<<clus-rs-ds-mgrp>>=
dat.cls1$grp <- rep(c(3, 4), each = 30)
dat.m <- rbind(dat.cls, dat.cls1)
...
<<dat-sr>>=
dat.sr <- datgen.sr(10, 3, cor = xch, rho = 0.9)
head(dat.sr)
@ 
...
Using the two data generation functions defined above, we provide a 
convenient function \code{simpower} to facilitate the power comparison 
of RGL and DS method for each test under a range of scenarios.
<<simpower>>=
...
<<sim-clus-rs>>=
simpower(1000, 0.05, FALSE, 10, 5, 0.0, c(0.1, 0.1), xch, 1, "cluster")
simpower(1000, 0.05, FALSE, 10, 5, 0.5, c(0.1, 0.1), xch, 1, "cluster")
@ 
...
individual is the unit of randomization. Each data set contains $40$
clusters. Two intra-cluster correlation
structures, exchangeable and AR1 (non-exchangeable), are used, each
with $\rho = 0.1$ and $0.9$. 
...
The group difference $\delta$ takes $0$, $0.3$ and $0.6$
respectively. 
...
&&&&& \multicolumn{2}{c}{$\delta = 0$} & \multicolumn{2}{c}{$\delta = 0.3$} & \multicolumn{2}{c}{$\delta = 0.6$} \\
  \cmidrule{6-7} \cmidrule{8-9} \cmidrule{10-11}
  Design & Corr. str. & $\rho$ & M & rate & RGL & DS & RGL & DS & RGL & DS \\
...
\begin{tabular}{rrrrrrrrrr}
  \toprule
  &&&& \multicolumn{2}{c}{$\delta = 0$} & \multicolumn{2}{c}{$\delta = 0.3$} & \multicolumn{2}{c}{$\delta = 0.6$} \\
  \cmidrule{5-6}\cmidrule{7-8}\cmidrule{9-10}
Design & Correlation structure & $\rho$ & Balance rate & RGL & DS & RGL & DS & RGL & DS \\
  \hline
  1 & Exchangeable & 0.1 & 1 & 0.05 & 0.05 & 0.94 & 0.94 & 1.00 & 1.00 \\ 

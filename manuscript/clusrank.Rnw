...
The default interface is called if \code{x} is a numeric vector 
and the synopsis is as the following:
<<args-cd, echo = FALSE>>=
args(clusrank:::cluswilcox.test.default)
@

The clustered rank-sum test is performed when \code{paired} 
is set to be \code{FALSE}.
...
clusters is greater than $50$ due to computational issue. The other arguments, \code{alternative} and \code{mu}, set the shape of the
...
<<datgen, echo=TRUE>>= 
library(mvtnorm)
datgen.rs <- function(nclus, maxclsize, delta = 0., rho = c(0.9, -.1), rate = 1,
...
head(dat0.cls)
@ 
To apply \code{``rgl''} method on this dataset with asymptotic test 
and exact test:
<<clus-rs-rgl>>=
cluswilcox.test(score ~ group(grp) + cluster(id), data = dat0.cls, method = "rgl")
cluswilcox.test(score ~ group(grp) + cluster(id), data = dat0.cls, method = "rgl", exact = TRUE)
@ 
To apply \code{``ds''} method on this dataset:
<<clus-rs-ds>>=
...
the norminal size, whereas when the data is unbalanced by randomly deleting half of the observations, the DS method cannot attain the nominal size. 

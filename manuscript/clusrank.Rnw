...
\code{wilcox.test}: {\bf blank line means new paragraph. Be careful.}
<<args-cd, echo=TRUE>>=
## args(stats:::wilcox.test.default)
args(clusrank:::cluswilcox.test.default)
@


As in the default interface of \code{wilcox.test}, \code{x} and
\code{y} are numeric vectors of data values, though unlike
\code{wilcox.test}, \code{y} is optional only for the clustered signed 
rank test where \code{x} and \code{y} may contain observations before
and after treatment and is spare for the clustered rank-sum test for which an
extra \code{group} variable is required to indicate the treatment
group assignment of the data values in \code{x}. 
The hypothesis related arguments \code{alternative}, \code{mu} and
\code{paired} are identical of those of the \code{wilcox.test}. 
The optional exact test (\code{exact = TRUE}) is only available for
clustered rank-sum test for RGL method and when the treatment group is
assigned at cluster level, otherwise \code{exact} should be set as
\code{FALSE}. Note that due to the computation issue, if exact test is
applicable, it
should not be applied to data sets which contain a large number of
clusters, typically $40$. Other arguments specific for clustered data include
...
The arguments of the formula interface is:
<<formula, echo=FALSE>>=
args(clusrank:::cluswilcox.test.formula)
@ 
{\bf no need for new paragraph}
The formula interface differs from the default interface in that a
formula is used to address the relationship between the input
variables and it is also different from the formula interface for the
\code{wilcox.test}. For clustered rank-sumtest, the left hand site is
the data values and the right hand side should comprise the
\code{group} id and the \code{cluster} id, additional covariate can
also be added if corresponding method allows, e.g., \code{stratum}. To
indicate the role of each variable, special functions have to be used
for \code{cluster} and \code{stratum} except for \code{group}, e.g.,
\code{z ~ group + cluster(cid) + stratum(sid)}  (see
Section~\ref{s:illus} for detail). {\bf captilize Section or Equation etc.}
...
cluster id. No group id is needed for signed-rank test. The option
\code{subset} and \code{na.action} have the same usage as they have in
the \code{model.frame} function to subset the data and 
determine the action to \code{NA} values in the data. 
The other arguments are identical to those in the default interface.


{\bf Please summarize 5 things you learned on latex and
  5 things on writing when we meet on Tuesday.}


\section{Illustrations}
\label{s:illus}

{\bf Use DS and RGL in the text consistently.}

\subsection{Rank-sum test} 

We use a scheme similar to the simulation study in 
\citet{Rosn:Glyn:Lee:exte:2006} to generate sample for clustered
rank-sum test, both balanced data and unbalanced data, though our
scheme is more straightforward and flexible. 
The sample is drawn in the fashion of a mixed effect model
which include a fixed effect of the treatment group, 
a random effect of the clustering drawn from a Normal  
distribution, and an individual Normal random error. 
The random component of the data values is first exponentiated before
adding the fixed effect.
The intra-cluster correlation is exchangable and the magnitude of
it can be further affected by the assigned treatment group. 
<<datgen, echo=TRUE>>= 
library(mvtnorm)
...
When \code{grplevel} is set as \code{"cluster"}, the generated data
frame contains \code{nclus} clusters under each treatment group with
corresponding intra-cluster correlation; whereas if \code{grplevel}
set as \code{"individual"}, a data set with individual level grouping
is obtained by randomly re-assigning the treatment id at indivudual
level. The maximum cluster size is set as \code{maxclsize}.  Argument
\code{rate} is used to control the unbalancedness of the data by
randomly keeping a proportion of \code{rate} of the orignal balanced
data where each cluster has the size \code{maxclsize}.  Therefore, to
genearte balanced data the \code{rate} can simply be set to $1$.  The
intra-cluster correlation of each treatment group is set through
\code{rho} which also enables a one-to-one correspondence between the
treatment and the magnitude of intra-cluster correlation.  The shift
in the location of the two treatment groups is set by \code{delta}.
The last argument, \code{grplevel}, is a switch of the grouping
level: if \code{grplevel = TRUE}, the grouping is assigned at cluster
level, otherwise, it is assigned at individual level. The generated
data set contains three columns: \code{score}, \code{grp} and
\code{id} which correspond to the observation, treatment group id and
cluster id.
...
and exact test:
<<clus-rs-rgl>>=
cluswilcox.test(score ~ grp + cluster(id), data = dat0.cls, method = "rgl")
cluswilcox.test(score ~ grp + cluster(id), data = dat0.cls, method = "rgl", 
                exact = TRUE)
@ 
To apply DS method on this dataset:
<<clus-rs-ds>>=
cluswilcox.test(score ~ grp + cluster(id), data = dat0.cls, method = "ds")
@ 


The next dataset involves individual level grouping.
{\bf why is the data called dat1.cls? I used cls at the beginning to
  denote cluster level group. I'd name this dataset d.ind.}
<<dat-rs-rgl-indi>>=
...
<<clus-rs-indi>>=
cluswilcox.test(score ~ grp + cluster(id), data = dat1.cls, method = "rgl")
cluswilcox.test(score ~ grp + cluster(id), data = dat1.cls, method = "ds")
@ 
...
{\bf use a single dataset to illustrate and let the user see what the
  function returns.}


\subsection{A simulation study}

{\bf are we doing the comparison that has not been done in the
  literature? That is something to highlight.}
...
{\bf refer to a table by its number. Never use the following table
  because it may not be following.}
The following table includes result from $10$ scenarios to compare the
performance of the two methods. The effect size in the power analysis
$\delta$ is set as $0.5$. Each of the first $8$ scenarios represents a
combination of the level at which the treatment is assigned, whether
the data set is balanced and $2$ level of intra-cluster correlation:
$\rho = 0.1$ and $\rho = 0.9$ where the treatment does not affect
$\rho$. There are $1000$ replications under each scenario, and each
data set contains $10$ clusters with a maximum cluster size $5$.  For
balanced data, each cluster size is $5$; for unbalanced data, the
unbalancedness is introduced by randomly deleting a half of the
corresponding balance data set. The last two scenarios aims at
studying the effect of variation in the cluster size by increasing the
maximum possible cluster size from $5$ to $10$ and randomly keep
$25\%$ of the original data instead of $50\%$.
...
{\bf anything interesting to compare for the signed-rank test?}
...
@ 
In both cases, the DS method has a higher power.  
{\bf we do not want to be misleading. Any comparison that has not been

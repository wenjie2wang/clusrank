...
also be accounted for.  Let $R_{ij}$ denote the rank of $X_{ij}$ among
all the observations, $R_{i+}$ denote the rank sum for the $i$th cluster.
The Wilcoxon rank sum statistic is as following:

\begin{equation}\label{eq:rglrs}
  W_{RGL} = \sum_{i=1}^N\sum_{j=1}^{g_i}\delta_{ij}R_{ij} = \sum_{i = 1}^N\delta_{i}R_{i+}
  \end{equation}

To derive the sampling distribution of $W_{RGL}$, the clusters are
further grouped according to their size, $g = 1, \ldots, \g_{max}$. Equation
\ref{eq:rglrs} can then be written as 
\begin{equation}\label{eq:rglrs1}
  W_{RGL} = \sum_{g=1}^{g_{max}}\sum_{i \in I_g}\delta_{i}R_{i+} = \sum_{j = 1}^{g_{max}}W_g,
\end{equation}
where $I_g$ is the set of indices of clusters whose size is $g$ and
$W_g = \sum_{i \ in I_g}\delta_iR_{i+}$.  Assume for each of these
...
m_g/N_g$. The null distribution of $W_{RGL}$ under permutation is
obtained by combining $W_g$'s. For large sample test, the test
statistic based on $W_{RGL}$ is
\begin{equation}\label{eq:rlgrs2}
  Z_{RGL} = \frac{W_{RGL} - E(W_{RGL})}{\sqrt{var(W_{RGL})}},
\end{equation}
where $E(W_{RGL}) = \sum E(W_g)$, and $var(W_{RGL}) = \sum var(W_g)$. 

In addition, if the data are stratified into $V$ strata, similar to
\ref{eq:rglrs1}, $W_{RGL}$ can be expressed as
\begin{equation*}
  W_{RGL} = \sum_{v = 1}^V\sum_{g = 1}^{g_{max}}\sum_{i \in I_{g,v}}\delta_iR_{i+},
  \end{equation*}
...
inefficiency. Additionally, the RGL perform best when correlation is
weak.

\subsection{DS rank sum test}
When the treatment is assigned at the subunit level instead of the
cluster level, the DS rank sum test can be applied. This method allows
arbitrary dependence structure within each cluster and also works when
treatment can affect the correlation structure. On the other hand,
however, making no assumption on the intra-cluster dependence
structure make it impossible to derive a small sample permutation
test, therefore this test can only be applied when sample size is
large. As a result, it cannot be applied to data which are strictly
contralateral, e.g., when all subjects under an eye study has one eye
under treatment $1$ and the other eye under treatment $2$, due to the
failure of the asymptotic theory of this method.
...
\begin{equation*} 
  W^* = \frac{1}{M+1}\sum^M_{i = 1}\delta_i^*R_i^*,
  \end{equation*}
where $\delta_i^*$ is the indicator of whether $X_I^*$ is under
treatment $1$ and $R_i^*$ is the rank of $X_i^*$ among the
pseudo-sample. Averaging $W^*$ over all possible pseudo-samples given
$\{X_{ij}, \delta_{ij}\}$, a large sample test statistic is
constructed as 
\begin{equation*}
  Z = \frac{S - E(S)}{\sqrt{\hat{var}(S)}},
  \end{equation*}
where $S = E(W^*|\mathbf{X}, \mathbf{\delta})$.

This method can also be generalized to the comparison of location
among $m$ treatment groups, $m \geq 3$, and the large sample test
...
\begin{equation*}
  W_c = \sum_{i = 1}^N\sum_{j=1}^g\delta_{ij}R_{ij},
  \end{equation*}
The sampling distribution of $W_c$ is derived by assuming that the
...
above randomization scheme. A large sample test is also available. A
note is that this test is equivalent to the RGL test for balanced
data when the treatment is assigned at the cluster level.
...
combination of estimated probability that an observation under
treatment $1$ is greater than that of an observation without treatment
$1$ within each cluster group, i.e., $\theta^{(g)} = Pr(X_{ij}^{(g)} >
X_{kl}^{(g)})$, where $X_{ij}^{(g)}$ is from treatment group $1$ and
$X_{kl}^{(g)}$ from treatment group $2$, the superscript refers to the
size of the cluster the observation belongs to. A common underlying
rank correlation is assumed across all clusters.

This test also works better when the intracluster correlation is weak
than when the correlation is strong.

\section{Wilcoxon signed rank test}\label{sec:signrank}
\subsection{Null hypothesis and general notation}
Let $X_{ij}$ be the pair-difference for the $j$th pair in the $i$th
cluster, $i = 1, \ldots, N$, $j = 1, \ldots, g_i$. We assume that the
clusters are independent while the pair-difference within each cluster
are correlated. The null hypothesis we want to test is that the
marginal distribution of $X_{ij}$ is symmetric around $0$, i.e., the
treatment does not shift the location of the distribution of the
baseline score.
...
\ldots, g_i\}$, $\V_{ij} = \text{sign}(X_{ij})$ and $S_{ij} =
V_{ij}R_{ij}$ be the signed rank.

For balanced data, the clustered WIlcoxon signed rank statistic is
defined as
\begin{equation*}\label{eq:rglsign}
  T_{RGL}^{(obs)} = \sum_{i=1}^NS_{i+} \equiv \sum_{i=1}^N\sum_{j=1}^gR_{ij}V_{ij},
  \end{equation*}
...
the sampling distribution of $T_{RGL}$ is then expressed by
\begin{equation*}
  T_{RGL} = \sum_{i=1}^N\delta_iS_{i+},
  \end{equation*}
where $\delta_i$ are i.i.d. random variable each taking on the values
$+1$ and $-1$ with equal probability under $H_0$. Therefore, if $N$ is small, a
permutation test can be performed by exhausting all possibilities of
$T_{RGL}$.  When $N$ is large, a large sample test statistic is
defined as
\begin{equation*}
  W_{RGL} = T_{RGL}\large/\sqrt(var(T_{RGL})),
  \end{equation*}
the asymptotic distribution of under $H_0$ is the standard normal
distribution.

For unbalanced data, \eqref{eq:rglsign} can be generalized as 
\begin{equation*}
  T_{RGL}^{(obs)} = \sum_{i=1}^Ng_i\bar{S}_i, \quad \text{where} \bar{S}_i = S_{i+}/g_i,
  \end{equation*}
which is a weighted average of $\bar{S}_{i}$ with weight in proportion
to the cluster size. A more efficient contruction, which is the one
implemented in our package, is
\begin{equation*}
  T_{RGL}^{(obs)} = \sum_{i=1}^Nw_i\bar{S}_i,
  \end{equation*}
where $w_i = 1/\var(\bar{S}_i)$ under $H_0$. Corresponding small
...
\begin{equation*
    R_i^* = \frac{1}{2}\left\{\sum_{i` = 1}^NI(|X_{i`}^*|\leq|X_i^*|) +{\sum_{i` = 1}^NI(|X_{i`}^*|<|X_i^*|  \right\}

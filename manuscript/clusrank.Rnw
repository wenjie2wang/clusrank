...
The package contains a real dataset named \code{eye} from a retrospective
...
The data contains 143 patients (283) eyes, among whom 62 had 
the rare variant and 81 did not have the rare variant.
So the clusters are the patients and the subunits are the eyes.
The outcome variable \code{CARMS} was the AMD grading score based 
...
The rare variant grouping is indicated by variable \code{Variant}. 
The Wilcox ranksum test is to be carried out on two subsets: 
...
implies strong evidence to the association between CFH~R1210C rare variant
and the severity of the symptom with GA as the advanced stage.
...
clusWilcox.test(CARMS ~ Variant + cluster(ID), data = eye, method = "ds",
                subset = CARMS %in% c(1, 2, 3, 5))
@
This time, the p-values of the two approaches are easily discernable.
...
@
The p-value remains virtually unchanged after controlling for the age/gender strata.

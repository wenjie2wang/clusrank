...
controlled by $\rho$, a numeric vector of length 2, one for each
...
and an exchangeable or AR1 conrrelation structure with correlation
parameter $\rho$, and $\delta$ is the pair difference.
...
tests through $1000$ replications on balanced data set. The significance level is set as
$0.05$. The true group differences is $0.$. Each data set contains
$10$ clusters with size $3$. The intra-cluster correlation structure
is exchangeable with parameter. For clustered rank-sum test, the
...
{\bf what is the last argument?  it should be named}
<<sim-clus-rs>>=
simpower(1000, 0.05, FALSE, 10, 5, 0.0, c(0.1, 0.1), ex, 0, clusgrp = TRUE)
@ 


For the clustered signed-rank tests, there is no explicit treatment
group, therefore we assign a universal correlation parameter to all
paired differences through a single numeric. Here we let \code{rho} be
$0.1$. Other arguments are set the same as the above simulation for
clustered rank-sum test, except that no \code{clusgrp} is needed since
there is no explicit treatment assignment for signed-rank test data. 

<<sim-sr>>=
simpower(1000, 0.05, TRUE, 10, 5, 0., c(0.1, 0.1), ex, 0)
@ 
...
\begin{tabular}{rrrrrrrrrr}
  ...
  ex  & 1 & 2 & 0.1, 0.1 & 4.3 & 4.7 & 17.5 & 17.9 & 64.3 & 65.3 \\ 
  ...
  ar1 & 1 & 2 & 0.1, 0.1 & 4.0 & 4.2 & 18.4 & 19.0 & 65.4 & 66.4 \\ 
...
\begin{tabular}{rrrrrrrrrr}
  ...
  \midrule
 ex  & 1 & 2 & 0.1, 0.1 & 4.0 & 4.3 & 17.5 & 16.7 & 69.2 & 67.8 \\ 
 ...
 ar1 & 1 & 2 & 0.1, 0.1 & 4.7 & 5.3 & 16.8 & 17.8 & 69.1 & 69.0 \\ 
...
For clustered signed-rank tests, we compare the RGL and DS methods
under $8$ designs. Each generated data set contains $40$ clusters with
a maximum cluster size $M = 5$. Two correlation structures,
exchangeable and AR1, are used, each with $\rho = 0.1$ and $0.9$. We
also compare the two tests with balanced and unbalanced data by
setting \code{rate} as $1$ and $0.5$. There are $1000$ replications
for each design.
...
\begin{tabular}{rrrrrrrrr}
  ...
  \midrule
  1   & 2 &  0.1, 0.1 & 5.2 & 5.4 & 37.7 & 38.3 & 98.0 & 98.0 \\ 
...
\begin{tabular}{rrrrrrrrr}
  ...
  Missing rate & M &   $\rho$ & RGL & DS & RGL & DS & RGL & DS \\
  \midrule
  1   & 2 &  0.1, 0.1  & 5.0 & 5.1 & 35.2 & 35.9 & 97.7 & 97.7 \\ 

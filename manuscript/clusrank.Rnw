...
The formula interface works for both rank sum test and signed rank test.
...
including \code{cluster} id and \code{group} id. Additional covariate
for specific method can also be added, e.g., \code{stratum} for
\code{rgl} method.
The \code{data} argument needs the name of a data frame
which should consist of the observed score and required covaraites.
The \code{subset} and \code{na.action} have the same effect as they have
in the \code{model.frame} function.
Other conventional arguments that can be tuned are \code{alternative},
\code{mu} and \code{exact}. Note that the exact permutation tests are only available for rank sum test using \code{rgl} method when the
treatment is assigned at cluster level and should be applied to sample with number of clusters less than 50.


Clustered signed rank test is performed when \code{paired} is set to
be \code{TRUE}.
For clustered signed rank test, the variable on the
left hand side of the formula should be the observed difference in
scores before and after the treatment and the right hand side of the
formula only requires the cluster id of each observation.
The exact permutation test is only available when the \code{rgl} method is used.
...
For the clustered rank sum test, besides \code{x}, \code{cluster} and \code{group} are required and an optional \code{stratum} id can be provided if using \code{rgl} method.
For the clustered signed rank test, \code{x} can be either the observed difference in scores or the observed score before treatment in addition to which the observed score after treatment is required as \code{y}. All other arguments can be set exactly the same as using the formula interface.
\red{FIXME: explain all the arguments;
separate ranksum and signed rank.}


\red{FIXME: at the very end, give the default interface
  similar to wilcox.test}



The package \pkg{clusrank} provides a single interface function for
both Wilcoxon rank sum test and signed rank for clustered data. The
function requires a dataframe. For clustered rank sum test, this data
frame should consist of cluster id, group id and observed scores (an
extra stratum variable for method \code{RGL}). For clustered signed
rank test, this data frame should consist of cluster id and observed
score before and after the treatment, or consist of cluster id and the
observed difference in scores before and after the treatment. For the
signed rank test, the argument \code{paired} should be set to
\code{TRUE}. Other conventional arguments that can be tuned are
\code{alternative}, \code{mu} and \code{exact}.
There are $2$ methods
provided for each of the test through setting the argument
\code{method} to \code{'rgl'} and \code{'ds'}.
Note that the exact
permutation tests are only available for RGL rank sum test when the
treatment is assigned at the cluster level and for RGL signed rank
test and should be applied to sample size which is larger than 50.
...
@
\red{FIXME: explain the arguments so that readers know what it does.}


The first datset has 10 clusters in each of the two groups,
...
@
\red{FIXME: can a user specify \code{method = c('rgl', 'ds')} to request
  both tests in one call?}

\red{FIXME: print from rgl method can be made more compact.}

\red{FIXME: can illustrate exact permutation test for this sample size.}


A simulation to compare the two methods become can be easily done:
<<simpower>>=
...
\red{FIXME: can add a small size study and small power study that has not been
done in the literature. need to make sure the sizes are fine before checking power.}
<<size>>=
...
\red{FIXME: can you do something similar? Use some simple function to generate data, illustrate usage of each function, and do some simulation that has not been done before if possible.}

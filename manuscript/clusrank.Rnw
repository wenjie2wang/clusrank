...
within-cluster dependence have been recently developed, but no existing
...
\red{Give more specifics of the eye study and use it as a motivating example.}
...
\citet{Datt:Satt:sign:2008} based on sampling within cluster performed
...
\citet{Datt:Satt:sign:2008} were implemented in this package.
...
The package contains a single interface function
\code{cluswilcox.test} which resembles the function \code{wilcox.test}
...
cluswilcox.test(z ~ cluster(id) + group(group), data = data,  method = 'rgl')
@ 
...
cluswilcox.test(z ~ cluster(id) + group(group) + stratum(strat), data = data)
@ 

Another interface of the clustered rank sum test is use a numerical
vector contains the observations as the first argument. A treatment
group id \code{group} is required and the \code{paired}
should be set to be \code{FALSE} (default) in addition.
<<rseg2, eval = FALSE, echo = TRUE>>=
cluswilcox.test(z, cluster = id, group = group, data = data, method = 'rgl', paired = FALSE)
@
The default of \code{paired} is \code{FALSE}.
 
For the signed rank test, there are two ways to input the data. First,
if the data are the observed difference between the two groups, e.g.,
say, dataframe \code{data} contains the observed difference \code{z}
and cluster id \code{id}, then the function is called in the following
way:
<<sreg, eval = FALSE, echo = TRUE>>=
data(crsd)
cluswilcox.test(z, cluster = id, data = data, paired = TRUE)
@ 
Or, if the observations from the two groups are saved in two vectors instead, say, \code{x} and \code{y}, 

...
\code{rho} can be generated as follows, \code{ex} for exchangeable 
...
The paird differences in a cluster are generated as
\[
...
size $3$, with exchangeble correlation parameter $\rho = 0.5$.
<<dat-sr>>=
...
Comparisons of the RGL method and the DS method under 
certain senarios have not been studied in the recent literature.
...
The function returns the empirical rejection rates of the RGL method
and the DS method for the setting defined by the inputs.


As an example, consider the rank-sum tests in a setting with cluster
level grouping, each group containing 20~clusters of size~3, with 
...
This is a good sign for the RGL method because the size is maintained
even when the exchangeable assumption is violated in this setting.
...
Table~\ref{tab:rs} show the results

...
clustered signed-rank test where \code{x} and \code{y} may contain
observations before and after treatment, it is not in use for the
clustered rank-sum test where the treatment group assignment of each
data value is provided through an extra numeric vector \code{group} as
the cluster id.  The hypothesis related arguments \code{alternative},
...
{\bf do we decline or issue a warning if number of cluster is greater
  40?}  {\bf try to start a new sentence at a new line}

...
The following table includes result from $8$ scenarios to compare the
performance of the two methods. Each scenario represents a combination
...
a half of the corresponding balance data set. 
...
especially when the intra-cluster correlation is large. 
<<rs-tab, results='asis', echo = FALSE, eval = TRUE>>=
library(xtable)
sum.tab <- rbind(clusb, clusub, indb, indub)
...
print(xtable(sum.tab), add.to.row = addtorow, include.colnames = FALSE,

...
For clustered data, although they have been developed
...
Usages of package \pkg{clusrank} are illustrated with simulated data
...
\proglang{R} codes for the DL methods are available from Dr. Datta's website
...
developed unifies unifies both RGL and DS methods under a user-friendly
...
The RGL Wilcoxon rank-sum test was designed for the scenario
where the treatment group is assigned at the cluster-level:
...
The package contains a real dataset named \code{eye} from an
ophthalmological study \citep{Ferr:Sedd:phen:2015}.
The objective of the study was to characterize the phenotype
associated with a rare variant in the complement factor
H~R1210C, a large protein.
The measurements were taken on both eyes of
...
from \code{Sex} and \code{Age_group}). There are $283$ subjects in the data set,


We apply both clustered rank sum test (RGL and DS) available in the
package to the data set \code{eye} when the CARMS grade of the subject
is one of $1$, $2$, $3$, or $4$.
<<eye1234-both>>=
data(eye)
...
implies a strong correlation between the presence of the
complement factor H R1210C variant and the severity of the symptom
with GA as the advanced stage.

Note that we can use the variable \code{Agesex} as an extra covariate
when applying the RGL method:
<<eye1234-rgl>>=
clusWilcox.test(CARMS ~ Variant + cluster(ID) + stratum(Agesex), data = eye,
                subset = CARMS %in% c(1, 2, 3, 4))
@
The p value of the RGL clustered rank sum test is still less than
$0.001$ after controlling for the effect of age and sex factor, which
is consistent to the test result when the factor is not controlled.


We also apply the two tests to the subset of the data when the CARMS
grade is one of $1$, $2$, $3$ and $5$ without considering other
additional information.
<<eye12355-both>>=
clusWilcox.test(CARMS ~ Variant + cluster(ID), data = eye, method = "rgl",
                subset = CARMS %in% c(1, 2, 3, 5))
clusWilcox.test(CARMS ~ Variant + cluster(ID), data = eye, method = "ds",
                subset = CARMS %in% c(1, 2, 3, 5))
@
The p values of the two approaches differ when the highest CARMS score is $5$, the p value of the RGL clustered rank sum test is $0.06455$ where as the DS clustered rank sum test reports a p value of $0.006312$.
Therefore the results of the two tests still suggest the correlation exists between the presence of complement factor H R1210C variant and the symptom with CNV as the advanced stage.
...
Further considering the effects of the age and sex using the \code{stratum} option for the RGL clustered rank test, we obtained similar result as when the age and sex effects were not considered.

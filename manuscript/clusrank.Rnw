...
\subsection{Datta and Satten (DS) method with grouping at
  cluster subunit-level}
...
\subsection{RGL method: cluster subunit-level grouping}
...
@
The p value of the RGL clustered rank sum test is less than $0.001$,
which implies a strong correlation between the presence of the
complement factor H R1210C variant and the severity of the symptom
with GA as the advanced stage, after controlling the age and sex
factors.
...
When the highest CARMS grade is $5$, i.e., the advanced stage is CNV
related, the p-value of the test is $0.064$, which provides weak
support to the hypothesis that the H R1210C variant is correlated with
the CNV related symptom.
\section{Discussion}
\label{s:disc}
...
We thank the Seddon Lab for the use of data for illustration. Rosner
and Lee were support in part by grant NIH R01EY022445 from the

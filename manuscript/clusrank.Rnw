...
Let $X_{ij}$ denote the $i$th observation in the $j$th cluster,
$1 \leq i \leq N$, $1 \leq j \leq g_i $. There are $N$ clusters
in total, the $i$th cluster contains $g_i$ observations.
Let $\delta_{ij}$ denote the treatment placed on $X_{ij}$,
$\delta{ij} = 1$ if $X_{ij}$ is under treatment $1$,
$\delta{ij} = 0$ if $X_{ij}$ is under treatment $2$.
(The DS method is able to compare $m$ groups.)

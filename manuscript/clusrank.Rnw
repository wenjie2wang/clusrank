...
<<dat-sr>>=
dat.sr <- datgen.sr(10, 3, cor = ex, rho = 0.9)
head(dat.sr)
@ 
...
<<sim-sr>>=
simpower(1000, 0.05, TRUE, 10, 5, 0., 0.1, ex, 0)
@ 
...
\begin{table}[tbp]
\centering
\caption{Empirical rejection rate comparisons for RGL and DS clustered signed-rank test, nominal level $5\%$, $N = 40$ clusters, maximum cluster size $M = 5$, $1000$ simulations for each combination.}
\begin{tabular}{rrrrrrrrrr}
  \toprule
  &&&& \multicolumn{2}{c}{$\delta = 0$} & \multicolumn{2}{c}{$\delta = 0.3$} & \multicolumn{2}{c}{$\delta = 0.6$} \\
  \cmidrule(lr){5-6}\cmidrule(lr){7-8}\cmidrule(lr){9-10}
Design & Correlation structure & $\rho$ & Missing rate & RGL & DS & RGL & DS & RGL & DS \\
  \hline
  1 & ex & 0.1 & 1 & 0.05 & 0.05 & 0.94 & 0.94 & 1.00 & 1.00 \\ 
  2 & & & 0.5 & 0.05 & 0.06 & 0.71 & 0.66 & 1.00 & 1.00 \\ 
  3 & & 0.9 & 1 & 0.05 & 0.06 & 0.50 & 0.50 & 0.97 & 0.97 \\ 
  4 & & & 0.5 & 0.03 & 0.03 & 0.46 & 0.47 & 0.96 & 0.96 \\ 
  5 & AR1 & 0.1 & 1 & 0.05 & 0.05 & 0.96 & 0.96 & 1.00 & 1.00 \\
  6 & & & 0.5 & 0.04 & 0.05 & 0.78 & 0.74 & 1.00 & 1.00 \\ 
  7 & & 0.9 & 1 & 0.05 & 0.05 & 0.50 & 0.51 & 0.98 & 0.98 \\ 
  8 & & & 0.5 & 0.05 & 0.05 & 0.52 & 0.51 & 0.96 & 0.96 \\ 
   \hline
\end{tabular}
\label{tab:sr}
\end{table}

...
\pkg{ClusterRankTest} package. Nonetheless, since some heavy computing tasks
are performed with \proglang{C++} code, our implemenation is faster.
...
\citep{Dutt:Datt:rank:2016} was recently added to the package
{\bf is ours faster?}, though it is

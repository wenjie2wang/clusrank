...
As an illustration, a balanced data set with $10$ clusters of size 5 with cluster level grouping is generated, the  :
<<dat-clus>>=
set.seed(1234)
...
<<clus-rs-rgl-asymp>>=
cluswilcox.test(score ~ grp + cluster(cid), data = dat.cls, method = "rgl")
@ 
...
<<clus-rs-ds>>=
cluswilcox.test(score ~ grp + cluster(cid)), data = dat.cls, method = "ds")
@ 
...
<<sr>>=
cluswilcox.test(score ~ cluster(cid).test(score ~ cluster(id), dat.sr, paired = TRUE, method = "ds")
@ 

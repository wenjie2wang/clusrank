...
@ 
Both methods have a empirical size that is close to the nominal
level.  This is a good sign for RGL since even as the RGL method
requires the exchangeability in the derivation of the asymptotic
distribution of the test statistic, it is quite robust to this
assumption when the correlation structure is AR1.


\subsection{Rank-sum tests}
A comparison of the two clustered rank-sum tests has been done by
\citet{Datt:Satt:rank:2005} when treatment is assigned to clusters. 
They mainly discussed the case in which the group membership is
correlated with the cluster size and DS method has more advantage in
these scenarios. 
No comparison has been done when the treatment is assigned at
individual level yet.   
Table~\ref{tab:rsclus} and Table~\ref{tab:rsind} show the results
from several designs using function \code{simpower} to compare the two
clustered rank-sum tests under different conditions when the unit of
randomization is cluster and individual respectively.  
Each generated data set contains $40$ clusters. Two intra-cluster correlation
structures, exchangeable and autocorrelated with order~1 (ar1), are
used with different level of correlation.
The maximum cluster size $M = \max_i n_i$ has two levels $2$ and $10$. 
To compare the performance of the two tests on balanced data and
unbalanced data, we set the random missing rate as $0$ and $0.5$.
...
For data with cluster-level grouping, both tests maintained the
nominal size, however, the DS method outperforms the 
RGL method in all situations we experimented, despite the cluster
size, intra-cluster correlation and the variation in the cluster
size. 
Both tests also maintained size close to the nominal level $0.05$ in
all scenarios. In contrast to the previous table, RGL method performs
better than the DS method under all the conditions we experimented
except for data with cluster size equals $2$ and with AR1 structure
which may due to the violation of the exchangeable assumption. 
...
\begin{table}[tbp]
  \centering
   \caption{Empirical rejection percentage comparisons for RGL and DS clustered signed-rank test with exchangeable and ar1 intra-cluster correlation, nominal level $5\%$, $N = 40$ clusters, $1000$ simulations for each combination.}
\begin{tabular}{rrrrrrrrrr}
  \toprule
  &&&& \multicolumn{2}{c}{$\delta = 0$} & \multicolumn{2}{c}{$\delta = 0.2$} & \multicolumn{2}{c}{$\delta = 0.$} \\
  \cmidrule(lr){5-6} \cmidrule(lr){7-8} \cmidrule(lr){9-10}
  Corr. str. & Missing rate & M &   $\rho$ & RGL & DS & RGL & DS & RGL & DS \\
  \midrule
 ex & 0   & 2 &  0.1, 0.1 & 5.2 & 5.4 & 37.7 & 38.3 & 98.0 & 98.0 \\ 

...
\leq i \leq N$, $1 \leq j \leq g_i $. There are $N$ clusters in total,
the $i$th cluster contains $g_i$ observations.  Let $\delta_{ij}$
...
\subsection{RGL method: unbalanced data}
The RGL method is desinged for the scenario in which the unit being
assigned treatment is cluster, e,g., when both eyes of the same
patient receive the same treatment. Therefore we denote the treatment
indicator for the $i$th cluster as $\delta_i$ for the illustration of
RGL method. The RGL method can be applied to balanced data (i.e., all
clusters have identical size) and unbalanced data (i.e., cluster size
varies). In addition, the effect of stratification, if exists, can
also be accounted for. 
Let $R_{ij}$ denote the rank of $X_{ij}$ among all observations.

The Wilcoxon rank sum statistic is 
\begin{equation}\label{eq:rglrs}
  W_{RGL} = \sum_{i=1}^N\sum_{j=1}^{g_i}\delta_{ij}R_{ij} = \sum_{i = 1}^N\delta_{i}R_{i+}
  \end{equation}
The idea behind the RGL method is to  stratify the clusters according
to the cluster size, compute 
The above equation can be then written as

The RGL method requires that all individuals in a single
cluster belong to the same treatment group, i.e., the
group membership indicators $\delta_{ij}$ for observations in
the $i$th cluster take a identical value, thus we denote the group membership
of the $i$th cluster as $\delta_{i}$.
Let $N_g$ and $m_g$ denote the number of clusters and number of clusters under
treatment 1 with cluster size $g$, respectivly, $g \in \{1, \ldots, g_{max}\}$.
It is also assumed
the observations within any single cluster are exchangable.
Let $R_{ij}$ denote the rank of each observation among all observations.
Then the test statistic $W_{rgl}$ is defined as
$$W_{rgl} = \sum_{i,j}\delta_{ij}R_[ij] = \sum_{i}\delta_iR_{i+}$$,
which is exactly the same as the standard Wilcoxon rank sum test.
The sampling distribution of this test statistic is derived by
conditioning on the cluster size and the number of clusters under treatment $1$
for that specific sample size, i.e., all the clusters are
first stratified based on their cluster size, then the sampling
distribution of each stratum is obtained. Because clusters
are assume to be independent, the expectation and
variance of $W_{rgl}$ are obtained by combining the corresponding
quantities of each stratum. Therefore,
\begin{equation*}
E(W_{rgl}) = \sum_{g = 1}^{g_{max}}m_g(R_{++,g} / N_g)
\end{equation*}
\begin{equation*}
Var(W_{rgl}) = \sum_{g = 1} ^ {g_{max}}[m_g(N_g - m_g) / \{N_g{N_g - 1}\}]
\sum_{i = 1}^{N_g}(R_{i+,g} - R_{++,g} / N_g)^2
\end{equation*}


RGL statistic is also capable of dealing with extra stratification
covariate coming with the scores, e.g., data from multiple medical centers where
each center can be treated as a stratum. The derivation of the
$E(W_{rgl})$ and $Var(W_{rgl})$ is very similar to the derivation for
unbalanced data. On the basis of the stratums defined by the cluster
size, each stratum is further stratified by the stratifying
covariate. The expectation and variance of the test statistic
restricted to each of these strata is computed by treating each
restriction as an ordinary Wilcoxon rank sum test. Assume there are
$V$ strata, the expectation and variance of $W_{rgl}$ are
\begin{equation*}
E(W_{rgl}) = \sum^{g_{max}}_{g = 1}\sum_{v =
1}^{V}m_{g,v}R_{++, g, v} / N_{g,v}
\end{equation*}
\begin{equation*}
Var(W_{rgl}) = \sum^{g_{max}}_{g=1}\sum_{v = 1}^V[m_{g,v}(N_{g,v} -
m_{g,v}) / \{N_{g,v}(N_{g,v} - 1)\}]\times\sum^{N_{g,v}}_{i=1}(R_{i+, g, v} - R_{++,g,v}/N_{g,v})^2
\end{equation*}
The large sample test statistic with asymptotic normality
is defined as $$Z_{rgl} = \frac{W_{rgl} - E(W_{rlg})}{VAR(W_{rgl})^{1/2}}$$.

\subsection{RGL method: with individual-level defined group membership}
The previous approach is only able to test when the group membership 
is determined at the cluster level. This assumption leaves out many other situation
for clustered data. An extension of the RGL method enables this method to consider
the cases where the group membership is determined at the individual level. 
The test statistic is still $W_{rgl}$, however, the derivation of the distribution 
of the test statistic is different. Besides conditioning on the cluster size,
the derivation now involves the conditioning on the number of observations 
under treatment $1$ in each cluster as well. The idea of the assumed sampling
scheme is to be illustrated based on balanced data. First, the clusters are 
randomly assigned to a specific exposure group with $q$ exposed subunits, according to 
the observed exposure distribution $Q$. Then for clusters with $Q_i = q$, a random $q$ 
out of $g$ subunits are randomized to be exposed and the remaining subunits are randomized
to be unexposed. (Exposed means under treatment 1.) Permutation test can be
derived based on this sampling scheme. 

For large sample test, the expectation and variance are as following:
\begin{equation*}
  E(W_c) = \{(gN + 1) / 2\}\sum^g_{q = 0}qN_q.
\end{equation*}

\begin{equation*}
  \hat{var}(W_c) = N[\{N/(N-1)\}\hat{var}(Q)s_B^2/g^2 + \hat{E}\{Q(g-Q)\}s^2_W/g].
\end{equation*}
where 

\subsection{DS method: 2 group}
DS construct another rank sum test based on a different idea from
RGL's method. A single individual from each cluster is sampled,
recorded as $X_i^*$ with its group membership $\delta_i^*$,
$1 \leq i \leq M$. A WIlcoxon rank-sum statistic can be defined as
\begin{equation*}
W^* = \frac{1}{M+1}\sum^M_{i=1}\delta^*_iR^*_i,
\end{equation*}

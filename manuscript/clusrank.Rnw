...
simpower <- function(nrep, level, paired, delta, rho, rate, nclus, maxclsize, ...) {
    do1rep <- function() {
        ...
        if( "grp" %in colnames(dat)) { # Signed-rank tests
            p.rgl <- cluswilcox.test(score, cluster = id, paired = FALSE, 
                                     data = dat, method = "rgl")$p.value
            p.ds  <- cluswilcox.test(score, cluster = id, paired = FALSE, 
...
<<power_sr>>=
simpower.sr(1000, 0.05, 1, 0.9, 1, 10, 5)
simpower.sr(1000, 0.05, 1, -0.1, 1, 10, 5)
@ 

...
K\red{FIXME: add discussion on informative cluster size;
...
The argument \code{y} is for clustered signed-rank test. 
There are $2$ methods available, \code{``rgl''} and \code{``ds''}.
Note that for \code{``rgl''} method, an extra \code{stratum} can be
supplied to account for the effect of stratification in the data, 
e.g., the effect of treatment center when data are collected from 
several centers. In addition, an exact test is carried out if 
\code{exact == TRUE}. Nevertheless, the exact test is only available for
\code{``rgl''} method when the grouping is at cluster level. In addition, the
exact test is not recommended for data with number of clusters is greater than
$50$. Other arguments, \code{alternative} and \code{mu} set the shape of the
alternative hypothesis and the null value respectively.

To run clustered signed-rank test, \code{paired} needs to be set as
...
observed scores as \code{x} and \code{y} respectively. Currently 
there is no method which can control the effect of stratification.
In addition, exact test is only available for \code{``rgl''} method.
Other arguments are set in the same way as for the clustered rank-sum test. 
...
observed paired difference and the right hand site comprises the cluster id, note that no group id is needed for signed-rank test.
...
same as those of the default interface.

...
\Abstract{
Rank-based tests are distribution-free alternatives to the
popular two-sample and paired t-tests.
For independent data, they are available in the built-in \pkg{stats}
package of \proglang{R}.
For clustered data, although there are some rank-based tests
developed recently, there does not exist an \proglang{R} package
with easy access.
We present a package \pkg{clusrank} where the latest developments are
implemented and wrapped under a unified user-friendly function.
With different methods dispatched based on the input data, this
package offers great flexibility in handling various situations such
as stratified data.
Exact tests based on permutations are also provided for some methods.
Details of the major schools of different methods are briefly
reviewed.
Usages of main functions of the package \pkg{clusrank} are illustrated
with simulated data.
The package also enables convenient comparison between selected
methods under settings that have not been studied before and the results are discussed.
}
...
Implementations of Wilcoxon rank-sum and signed-rank tests for independent
data have been widely available in standard software packages, such as
function \code{wilcox.test} in the built-in \proglang{R} package
\pkg{stats}, and \code{PROC NPAR1WAY} in \proglang{SAS}.
...
from observations of the same subject are not independent but correlated.
...
variances of the test statistics are underestimated, and as a
consequence, the resulting p-values are greater than what they should be.
...
from the same cluster (i.e., subject) belong to the same treatment
group,
...
test after ranking all the observations combined, they derived
aymptotic mean and the variance under the clustered setting that
...
signed-rank test to clustered data by adjusting the variance of the
standard test statistic, assuming a common intercluster correlation
across clusters.
...
sampling members within cluster that accounts for informative cluster sizes.
...
The package \pkg{clusrank} we developed made first CRAN appearance in December, 2015.
The package provides easy computations and comparisons of these recently available rank-sum tests
...
Note that the RGL methods were
...
the \proglang{R} built-in package \pkg{stats}, the package
\pkg{clusrank} we developed unifies unifies both RGL and DS methods
under a user-friendly interface that accommodates the specifications from these methods.
The \pkg{clusrank} package makes it very easy for users to compare the
performances of different approaches under various settings; see
Section~\ref{s:simu}.


The rest of the article is organized as follows.
The recently available Wilcoxon rank-sum tests and signed-rank tests
for clustered data are briefly reviewed in Sections~\ref{s:ranksum} and
~\ref{s:signedrank},  respectively.
...
In Section~\ref{s:simu}, comparisons of selected methods that have not
been studied previously are reported, which
...
of the measures of location of the two groups, i.e.,
the distribution of $X_{ij}$ remains the same regardless
of the group indicator $\delta_{ij}$.


\subsection{Rosner, Glynn, and Lee (RGL) method with grouping at the cluster-level}
The RGL Wilcoxon rank-sum test was designed for the scenario where the
treatment group is assigned at the cluster level:
i.e., $\delta_{ij} = \delta_i$ for all $1 \le j \le n_i$.
...
individual observations within each cluster are exchangeable and that the
...
distribution of $Z$ as $N \to \infty$ is standard normal under
mild conditions.
The method can be extended to the case of stratified data.


RGL (2003) compares groups at each cluster size, the
...
\subsection{Datta and Satten (DS) method with grouping at individual
...
the test statistics, using mid-rank to allow for ties in the data.
...
individual cluster-member-level grouping; i.e., for each cluster $i$,
...
Note that as long as members from the same cluster have an identical
distribution, the DS method does not require the distribution of
members from clusters with different sizes to be identical, while this is
usually a common assumption made by most signed-rank tests for
clustered data in the literature.
...
rank-sum test when the treatment group is assigned at individual cluster member level due to
computational complication.  Note that an exact test is very computationally
intensive, so it is recommended to be used only with small samples.



The main function specifies the cluster by a \code{cluster} variable
indicating which cluster each observations belongs to.
Additionally, if the dataset is stratified, a numeric vector
\code{strata} can be provided to indicate stratum membership of each
observation.
Other arguments specific for clustered data include
\code{cluster} which should be a numeric vector containing the cluster
id. For clustered rank-sum test, the argument \code{group} should be a
numeric vector containing the treatment group id for each observation. In
...
the cluster id, \code{cluster} and the treatment assignment,
\code{group}. For both tests, the two methods, RGL and DS, are available by
specifying \code{method} as \code{rgl} or \code{ds}.
...
cluster id. Neither \code{group} or \code{stratum} is applicable for signed-rank tests.
...
As an example, consider the rank-sum tests in a setting with cluster-level grouping, each group containing 20~clusters of size~3, with
...
cluster & ex  & 0 & 2 & 0.1, 0.1 & 4.3 & 4.7 & 17.5 & 17.9 & 64.3 & 65.3 \\
 ...
 & ar1 & 0 & 2 & 0.1, 0.1 & 4.0 & 4.2 & 18.4 & 19.0 & 65.4 & 66.4 \\
  ...
  observation & ex  & 0 & 2 & 0.1, 0.1 & 4.0 & 4.3 & 17.5 & 16.7 & 69.2 & 67.8 \\
  member &&&&           0.5, 0.5 & 4.6 & 5.0 & 19.8 & 19.6 & 69.1 & 68.0 \\
...
individual cluster member level.
...
the RGL method has higher power with individual cluster-member-level group assignment.
...
when the treatment group is assigned at the individual cluster-member level.
...
Package \pkg{clusrank} provides two schools, the RGL method and the DS
...
individual in each group within a cluster;
both asymptotic tests require that the number of clusters to not to be too small.


Implementation of other rank-based methods for clusterd data can be
considered in future development of package \pkg{clusrank}.
...
available in package \pkg{ClusterRankTest} \citep{Dutt:Datt:clus:2016}.
...
\subsection{Data}
We illustrate the usage of the package \pkg{clusrank} by applying the
clustered Wilcoxon test to an eye study data set
\cite{Ferr:Sedd:phen:2015}. The aim of the study was to characterize
the phenotype associated with a rare variant in the complement factor
H R1210C, a large protein. The measurements were taken on both eyes of
each subject, therefore the measurements are clustered.
The pheonotypic features under study are categorized by the
Age-Related Maculopathy Staging (CARMS) score which have $5$ levels,
where the first $3$ levels are corresponding to the severity of the
development of the symptoms in an increasing order, and level $4$ and
level $5$ are corresponding to two different types of advanced
symptoms, where level $4$ indicates the presence of geographic
atrophy (GA) and level $5$ indicates the presence of choroidal
neovascularization (CNV) related symptoms.
Therefore the test is to be carried out on two subsets: the subset of
observations with CARMS grade 1, 2, 3 or 4,
and the subset of observations with CARMS grade 1, 2, 3 or 5.
Additional information such as age and sex is also provided.
This data set comes with the package with the name \code{eye} and
contains $7$ variables: \code{ID} (patient ID), \code{Eye} (factor
indicating the
position of the eye being measured), \code{Variant} (indicator of whether
there is H R1210C variant), \code{CARMS}, \code{Sex},
\code{Age_group}, and \code{Agesex} (a composite score constructed
from \code{Sex} and \code{Age_group}).

We apply the clustered rank sum test (RGL) to the data set \code{eye}
when the CARMS grade of the subject is one of 1, 2, 3, or 4, and use
the variable \code{Agesex} as an extra covariate:
<<eye1234-rgl>>=
data(eye)
clusWilcox.test(CARMS ~ Variant + cluster(ID) + stratum(Agesex), data = eye,
                subset = CARMS %in% c(1, 2, 3, 4))
@
The p value of the RGL clustered rank sum test is less than $0.001$, which implies a strong correlation between the presence of the complement factor H R1210C variant and the severity of the symptom with GA as the advanced stage, after controlling the age and sex factors.
...
When the highest CARMS grade is $5$, i.e., the advanced stage is CNV related, the p-value of the test is $0.064$, which provides weak support to the hypothesis that the H R1210C variant is correlated with the CNV related symptom.

\section{Acknowledgement}

We thank the Seddon Lab for the use of data for illustration. Rosner and Lee were support in part by grant NIH R01EY022445 from the National Eye Institute.

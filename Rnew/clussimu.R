library(MASS)
clus.simu.rks.2 <- function(M, rho, n, delta, pc) {
    ...
    ## for cluster under each treatment.
    ## n: A numerical vector, the cluster sizes.
    ## delta: The location shift between two groups.
    ## pc: A numerical vector or a matrix, the probability distribution of
    ## treatment of each cluster size. If it is a matrix, each row represent a treatment group
...
do1.clus.simu.rks.2 <- function(M, rho, n, delta, pc) {
    dat <- clus.simu.rks.2(M, rho, n, delta, pc)
    dat$group <- dat$group + 1
    p1 <- cluswilcox.test(X, cluster = c.id, group = group, data = dat)$p.value
    p2 <- cluswilcox.test(X, cluster = c.id, group = group, data = dat, method = "ds")$p.value
    c(p1, p2)
}

power.clus.simu.rks.2 <- function(B, slevel, M, rho, n, delta, pc) {
    dat <- t(replicate(B, do1.clus.simu.rks.2(M, rho, n, delta, pc)))
    apply(dat, 2, function(x) mean(x < slevel))
    }
...
clus.simu.sr1 <- function(M, n, delta, rho) {
    ...
    ## rho: rho is suppposed to have the length of the number of clusters
    if(length(rho) == 1) {
        rho <- rep(rho, sum(M))
    }
...
clus.simu.sr2 <- function(M, n, delta, rho, phi, gamma) {
    ...
    ## Arguments:
    ##  M:  a numeric, contains the number of clusters for each size
    ##  n:  a numeric, possible cluster size
    ## rho: a numeric
    ...
    cluster <- rep.int(1 : sum(M), times = clus.size)
    if(length(phi) != n) {
        stop("phi should have the size of cluster size")
    }
    if(length(gamma) != length(unique(phi))) {
    ...
    B<- rnorm(length(gamma) * M) * sqrt(gamma)
    B <- rep.int(B, times = rep(phi.tab, M))
...
clus.simu.sr.info <- function(M, n, beta, delta, rho) {
    ##  data generating model in DS paper signed rank test with
    ##  informative cluster size and sign.
    ## informative cluster size (clusters with the same cluster size have the same sign)
    ## Xij = sign(Hij)exp(|Hij|)
    ## Hij = Wi + eij, Wi ~N(delta, rho_i), eij ~ N(0, 1 - rho_i)
    ## Arguments:
    ...
    sn <- rep.int(sn, times = cluster.size)
    ...
    cluster <- rep.int(1 : sum(M), times = cluster.size)
...
do1.sr1 <- function(M, n, delta, rho) {
    ## Simplest data generating model in DS paper signed rank test
    ## Xij = sign(Hij)exp(|Hij|)
    ## Hij = Wi + eij, Wi ~N(delta, rho_i), eij ~ N(0, 1 - rho_i)
    ## Arguments:
    ...
    p1 <- cluswilcox.test(X, cluster = c.id, data = dat, paired = TRUE) $ p.value
    p2 <- cluswilcox.test(X, cluster = c.id, data = dat, paired = TRUE, method = "ds") $ p.value
    c(p1, p2)
}

power.sr1 <- function(B, slevel, M, n, delta, rho) {
    dat <- replicate(B, do1.sr1(M, n, delta, rho))
    return(apply(dat, 1, function(x) mean(x < slevel)))
}

do1.sr2 <- function(M, n, delta, rho, phi, gamma) {
    ## Simplest data generating model in DS paper signed rank test
    ## intracluster ecorrelation is not exchangable
    ## Xij = sign(Hij)exp(|Hij|)
    ## Hij = Wi + B_phij + eij, Wi ~N(delta, rho_i), eij ~ N(0, 1 - rho_i, gamma j)
    ...
    dat <- clus.simu.sr2(M, n, delta, rho, phi, gamma)
    p1 <- cluswilcox.test(X, cluster = c.id, data = dat, paired = TRUE) $ p.value
    p2 <- cluswilcox.test(X, cluster = c.id, data = dat, paired = TRUE, method = "ds") $ p.value
    c(p1, p2)
}
...
power.sr2 <- function(B, slevel, M, n, delta, rho, phi, gamma) {
    dat <- replicate(B, do1.sr2(M, n, delta, rho, phi, gamma))
    return(apply(dat, 1, function(x) mean(x < slevel)))
}


do1.sr.info <- function(M, n, beta, delta, rho) {
    ##  data generating model in DS paper signed rank test with
    ##  informative cluster size and sign.
    ## informative cluster size (clusters with the same cluster size have the same sign)
    ## Xij = sign(Hij)exp(|Hij|)
    ...
    ##      determine the sample size and sign of the observation
    ##  rho: rho is suppposed to have the length of hte number of clusters
    dat <- clus.simu.sr.info(M, n, beta, delta, rho)
    p1 <- cluswilcox.test(X, cluster = c.id, data = dat, paired = TRUE) $ p.value
    p2 <- cluswilcox.test(X, cluster = c.id, data = dat, paired = TRUE, method = "ds") $ p.value
    c(p1, p2)
}

power.sr.info <- function(B, slevel, M, n, beta, delta, rho) {
    dat <- replicate(B, do1.sr.info(M, n, beta, delta, rho))
    return(apply(dat, 1, function(x) mean(x < slevel)))
}

cluswilcox.test.ranksum.rgl.clus <- function(x, cluster, group, strat, exact,  alternative) {
## The input data should be already in the increasing order with cluster.
    ...
    strt <- stats::aggregate(strat ~ cluster, FUN = mean)[, 2]
    clus <- unique(cluster)
    ...
    ## mgv: number of clusters under trt X with csize g in stratum v
    ## ngv: number of clusters under trt Y with csize g in stratum v
    ## Ngv: number of clusters with csize g in stratum v
    ## Rsumgv: rank sum of clusters with size g in stratum v
    ## VRgv: variance of rank of clusters with size g in stratum v
    ...
    VarWc <- sum(mgv * ngv / (Ngv ^ 3 * (Ngv - 1)) * VRgv)
...
cluswilcox.ranksum.rgl.sub <- function(x, cluster, group, exact, alternative) {
    ...
    if(bal == TRUE) {
        ...
        VarWc <- n.clus * ((n.clus / (n.clus - 1)) * varQ * sb.2 / csize ^ 2) + sum(q.uniq * (csize - q.uniq) * nq) * sw.2 / csize
        Zc <- (Wc - EWc) / sqrt(VarWc)
    } else {
